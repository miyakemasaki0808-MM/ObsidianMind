...
/** 「どの分野にも当たらない」を表す機械用の値。**ユーザーに見せるカテゴリではない**（→ 判断13）。 */
const val NOTE_FIELD_NONE_ID = "NONE"

/**
 * 語彙の版。**候補集合を変えたら必ず上げる。**
 *
 * 入力指紋に含めるので（→ 判断9）、上げ忘れると**リストを更新しても古い結果が出続ける**。
 * これは入力指紋キャッシュで最も気づきにくい失敗の型なので、
 * **語彙を触る差分では必ずこの行も動く**という形にしてある（同じファイルに置く）。
 */
const val NOTE_FIELD_VOCABULARY_VERSION = 1
