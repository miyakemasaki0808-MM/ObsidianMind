 ...
 * 「戻れば同じ10枚が同じページ位置で残る」は束とページ位置の2つで1つの条件なので、
 * ページ位置だけを画面ローカルに置くと、**`冊子 → ノート → 戻る` でページ位置だけが失われる。**
...
/**
 * 編めない理由。**2つを畳まない** — 「探している最中」と「見つからなかった」は
 * 待てば変わるかどうかが違い、利用者の次の行動が変わる。
 *
 * 「取れなかった」は無い。関連ノートは AI が失敗しても決定的チャンネルの候補を返し、失敗の状態を持たない。
 */
