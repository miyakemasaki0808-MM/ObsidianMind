...
// （蒸留が parseDistillResponseIds で候補IDと照合するのと同じ強さ → features/reflect_remark.md 判断5）。
...
internal object RemarkLimits {
     ...
     *
     * 返事は「保存は原文・AIへは抜粋」で分けるので、AIへ渡す側の上限
     * （[REPLY_EXCERPT_CHARS]）からここを決めない（→ features/reflect_remark.md 判断6）。
     */
...
internal enum class RemarkRejection {
     ...
     *
     * プロンプトは「問い**か**関連ノート接続のどちらか一方」を求めているが、モデルは
     * 「…どのような役割を果たすのか、具体的に考えてみましょう。[[候補A]]」
     * のように**問いの末尾へリンクを足しただけ**の出力を返すことがある。
 ...
 *
 * 捕まえたいのは「…果たすのか、具体的に考えてみましょう。[[候補A]]」の形で、
...
internal fun composeRemark(
    ...
    // **リンクの有無で免除しない。** 実在ノートへのリンクをノート固有の参照とみなして免除すると、
 ...
 *
 * 痕跡の俯瞰要約は読み手自身の行動を述べる文なので二人称が要るが、
