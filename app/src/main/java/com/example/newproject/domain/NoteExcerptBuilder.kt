 ...
 *
 * 超過時は既存Markdownパーサの仕様により、frontmatter除去、コードフェンス言語名の除去、
 * 段落内改行の空白化、箇条書き記号の `-` への統一が起きる。**リストの番号と入れ子段数は
 * 保持される**（以前は ordered list が箇条書きへ潰れ、手順書の順序がモデルへ届かなかった）。
 * 短いノートとの非対称は、超過した入力だけを圧縮する仕様として受け入れる。
 *
