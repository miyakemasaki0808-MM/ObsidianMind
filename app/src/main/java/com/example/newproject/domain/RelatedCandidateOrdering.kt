...
/**
 * AIへ渡す候補タイトルを、除外・並べ替え・上限適用して返す純ロジック。
 *
...
internal fun orderRelatedCandidateTitles(
    ...
    val pool = candidateTitles.filterNot { it.toNormalizedObsidianTitle() in excluded }
    ...
    val sameGroup = pool.filter { extractHexPrefix(it)?.take(2) == twoDigit }
    val sameCategory = pool.filter {
        val p = extractHexPrefix(it) ?: return@filter false
        p.take(1) == oneDigit && p.take(2) != twoDigit
    }
    val noPrefix = pool.filter { extractHexPrefix(it) == null }
