 ...
 *
 * **画面を分けるのは、AIタブのパネルに置くと埋もれるため。** 要約 → 蒸留 → ひとこと
 * という長い同一スクロールの最下段で、いちばん短い結果がいちばん見つからない。
 *
 * **返事欄があって初めて一周する。** 1文だけ返して終わると AI が問いを投げて会話を打ち切る形になる。
 * 返事を書けることで**読む → 問いが返る → 自分の言葉を残す → 次の再会で読み返す**になる。
 * **返事はAIへ再送しない**（→ features/reflect_remark.md §2 非ゴール）。
 *
