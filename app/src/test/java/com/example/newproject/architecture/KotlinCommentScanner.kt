...
internal object KotlinCommentScanner {
     ...
     *
     * **枝番の無い番号（`P1`）は、レビューの文脈と並んだときだけ数える**（`前回P1`・`レビューのP2`・`P0指摘`）。
     * `P1` 単独を落とすと「始点P1と終点P2」のような正当な説明まで落ちる。
     * 捕まえるのは番号の形だけで、「前のレビューで」のような言い回しの経緯は通る。
     */
    private val HISTORY_MARK = Regex(
        """\b20\d{2}-\d{2}-\d{2}\b|\bP[0-3]-\d+\b""" +
            """|(?:前々回|前回|今回|レビュー|指摘|修正確認)(?:の|で|は|：|:)?\s*P[0-3](?![-\d])""" +
            """|(?<![A-Za-z0-9])P[0-3](?![-\d])\s*の?指摘"""
    )
