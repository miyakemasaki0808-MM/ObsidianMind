...
class NoteFieldInputVersionTest {
    ...
    fun `同じ入力なら同じ指紋になる`() {
        assertEquals(
            noteFieldInputVersion(excerpt, NoteField.Technical),
            noteFieldInputVersion(excerpt, NoteField.Technical)
        )
    }
    ...
    fun `本文が変われば指紋が変わる`() {
        assertNotEquals(
            noteFieldInputVersion(excerpt, NoteField.Technical),
            noteFieldInputVersion(NoteExcerpt("料理のメモ", isAbridged = false), NoteField.Technical)
        )
    }
    ...
    @Test
    fun `ヒントが変われば指紋が変わる`() {
        val withHint = noteFieldInputVersion(excerpt, NoteField.Technical)
        val otherHint = noteFieldInputVersion(excerpt, NoteField.Learning)
        val noHint = noteFieldInputVersion(excerpt, null)
    ...
    fun `省略の有無で指紋が変わる`() {
        assertNotEquals(
            noteFieldInputVersion(excerpt, null),
            noteFieldInputVersion(NoteExcerpt(excerpt.text, isAbridged = true), null)
        )
    }
}
