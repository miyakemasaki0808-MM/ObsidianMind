...
│   │   ├── controller/
│   │   │   ├── NoteSessionCoordinator.kt       # 7 Controllerの生成と横断調停・Vault世代（Android API非依存）
