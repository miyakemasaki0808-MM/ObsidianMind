...
残る既知未達は下部ナビ帯の上のバッジ**塗り**1件（→ `_wip` の A11Y-1）。
**2026-07-31 にエミュレータで5画面＋ダークモードを一巡し、確認の過程で判断7（暗幕→白ヘイズ）と
ボタン位置の是正を行った。ダークは据え置きのとおり変化なし。**
**2026-08-02 に判断8を追加**（バッジの中の記号は文字ではなく記号なので 1.4.11 の3:1を当てる）。
同日、塗り側を輪郭線で解こうとして差し戻しており、**やらないことの記録は
[ui_design_principles](ui_design_principles.md) §3 が持つ**。見た目に触る前はそちらを先に読む。
...
### 判断8. バッジの記号は「文字」ではなく「記号」として扱う（2026-08-02）

2026-07-29 に、緑の塗りの上の白い「✓」が 2.49 だったため**黒へ変えた**（判断3の系）。
数値は 5.53 になったが、**10dp程度のバッジでは黒い記号が面積の大半を占めて重く見える**。

見直しの結果、**そもそも当てる基準が違っていた**。
...
3:1 を当てると、**現在の緑 `#109384` の上で白は 3.80 で通る**（2026-07-29 に 2.49 だったのは
当時の緑が `#16B8A6` と明るかったため。緑を暗くした結果、同じ白の条件が変わっていた）。

**同じ塗りの上で前景が2つに割れる。** `buttonSecondary` はボタンの塗りでもバッジの塗りでもあり、
ラベルは黒（5.53）、記号は白（3.80）になる。これを1つに揃えると、
**記号側が不必要に 4.5:1 で制約される**。そこで `onStatusBadge` を新設した。

**Success と Error で共有し、明暗で反転する。**

| | 緑 `buttonSecondary` 上 | 赤 `errorSurface` 上 |
|---|---:|---:|
| ライト（白の記号） | 3.80 | 5.89 |
| ダーク（黒の記号） | 8.44 | 7.57 |

ダークで白へ戻すと 2.49／2.78 で 3:1 すら割るため、**明暗で反転させる**必要がある
（判断7の見出しと同じ形）。副次的に、**ライトで Error が白・Success が黒という不揃いが解消**した
（Error 側は元から `onErrorSurface` が白だった）。

**教訓:** 判断3「塗りと文字は別基準なので同じ色を共有できない」は正しかったが、
**「文字」の側をさらに分ける必要があった**。同じ塗りに載るものでも、
読む対象か示す対象かで基準が変わる。**基準の当て方を間違えると、
満たす必要のない制約を自分に課したまま見た目を犠牲にする。**

なお同日、この未達を輪郭線で解こうとして差し戻している（→ [ui_design_principles](ui_design_principles.md) §3）。
**A11Y-1（塗り自体がナビ帯の上で 3:1 に届かない）は未解決のまま**で、判断8はその中身の話である。
