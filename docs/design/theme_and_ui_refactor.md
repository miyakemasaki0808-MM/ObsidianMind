...
### 判断8. ナビ帯上のバッジも輪郭線で解く（2026-08-02・A11Y-1）

AIタブのSuccess/Errorバッジは、パネル上の塗りボタンと違って**下部ナビ帯**の上に載る。
ライトのナビ帯はブランド色 `Indigo` そのもので、彩度が高く相対輝度も中位のため、
塗りだけでは3:1（非文字基準）を出せない（実測 Success 1.61・Error 1.04）。
判断4（パネル上の塗りボタン）が使う対応表は「パネル」しか数えておらず、
**ナビ帯という別の面に載る部品を検証対象に含めていなかった**のが未達の直接の原因。

ナビ帯の色やバッジの塗りそのものを変える案は、[background_ai_ux](background_ai_ux.md) の
配色決定とブランド色（Indigo）そのものへ踏み込むため見送った。**判断7と同じ形**
（文字色では解けない箇所を面の設計で解く）に倣い、**輪郭線で境界を作る**。

`buttonOutlineOnGradient`（グラデーション直上のボタンが同じ理由で持つ輪郭線）と
同じ値 `LogoNavy` を再利用した。ナビ帯（Indigo）に対して 3.15 で基準を満たし、
新しいブランド色を増やさずに済む。ダークのナビ帯上ではSuccess/Errorの塗り自体が
元から3:1を満たす（5.93／5.32）ため、`buttonOutlineOnGradient` と同様に透明のままにした。

**A11Y-1 の未修正判断（優先度低・規模感小〜中）自体は覆っていない。** 覆したのは
「直すならナビ帯の色かバッジの明度に踏み込むしかない」という前提で、
実際には既存の輪郭線パターンをもう1箇所へ足すだけで済んだ。
