...
**未解決:** [current_issues](../_wip/current_issues.md) 2-9（依存の循環3組）・2-10（`NoteViewModel` の依存内部生成でテスト不能）・2-11（単一 UiState を6 Controller が共有所有）。**パッケージは整理されたが依存の「向き」はまだ制約されていない。**
