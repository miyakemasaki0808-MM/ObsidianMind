...
**状態:** 実装済み。Controller共通化は2026-07-26の5件目再判定まで含めて決着（**共通化せず、相似のまま維持**。以後は件数をトリガーにしない）。非同期の世代IDは二層（Vault単位＝共有／ノート単位＝Controller自前）で確定。2026-07-27に依存生成・状態所有・パッケージ依存の境界を型とJVMテストで固定し、AI本文の切り出し責務も同じ依存方向に沿って呼び出し側へ移した。2026-08-01に、この依存固定が `android.*` を数えていなかったこと（`model` に `Uri` が残る）を明らかにし、対処は別文書へ切り出した。
