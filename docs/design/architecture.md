...
**状態:** 実装済み。Controller共通化は2026-07-26の5件目再判定まで含めて決着（**共通化せず、相似のまま維持**。以後は件数をトリガーにしない）。非同期の世代IDは二層（Vault単位＝共有／ノート単位＝Controller自前）で確定。2026-07-27に依存生成・状態所有・パッケージ依存の境界を型とJVMテストで固定し、AI本文の切り出し責務も同じ依存方向に沿って呼び出し側へ移した。
...
この型が保証するのは「生の `String` を誤って直接渡さない」ことまでで、同一Gradleモジュール内の生成箇所を封じるものではない。予算内はMarkdownを解析せず原文をそのまま返し、超過時だけ純関数で圧縮する。単一巨大ブロック、コードフェンス復元、全体からの見出し均等選抜、注意書きを含む予算不変条件をJVMテストで固定した。これは判断3「壊れやすい文字列処理をAndroid I/Oから分離する」の横展開にあたる。

レビュー時の実測で、最大1MBの本文をMarkdown解析するとデスクトップJVMでも数百msを要し、
呼び出し元の `viewModelScope`（Main）を占有することが判明した。そこで要約・関連・補記・
クイズ・セクション3経路の抜粋生成だけを `Dispatchers.Default` へ移し、AI生成前の
タップやノート切替を止めないようにした。各クラスは本番既定値を `Default` とする
`excerptDispatcher` を持ち、JVMテストではテストスケジューラへ差し替える。5ファイル・
7呼び出しが必ずこの境界内にあることはソース走査テストで固定する。

注意書きは、正規化後に全ブロックが収まって `(omitted)` が存在しない場合も正しく読めるよう、
「`(omitted)` は存在する場合に省略区間を示す」という条件付きの文言にした。また注意書きと
本文をつなぐ改行を含む `ABRIDGED_NOTICE_PREFIX` を描画と予算計算で共有し、AIへ渡す
本文由来領域全体が用途別上限を1文字も超えない不変条件へ揃えた。
