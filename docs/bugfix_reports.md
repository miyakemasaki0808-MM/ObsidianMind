...
**AI（最新モデルを含む）が生成しがちで、通常フローでは表面化しない潜在バグ**を、再現経路・根本原因・修正・教訓つきで記録する累積文書。当初は状態遷移（画面遷移・Fold開閉によるActivity再生成）をまたいだときに **UIのタップ導線が意図した処理に到達しない** 型を中心に集めたが、#3 で **AI応答の解釈が正常系だけを想定し、崩れた出力で破綻する** 型、#4 で **確認した値と使う値がズレて稀なタイミングだけ別対象へ書く** 型も収録した。
...
| [#4](#4-vault切替の混入対策が効いていない確認後に同じ値を読み直す) | 旧ノートの読書痕跡が切替後の新Vaultへ書かれる | check-then-act で確認後に同じ値を読み直す | 痕跡保存の起動直後にVault切替 | Claude Opus（ReadingTrace のVault分離を修正した際） | Codex（レビュー） |
...
## #4 Vault切替の混入対策が効いていない（確認後に同じ値を読み直す）

**混入者:** Claude Opus（ReadingTrace のVault分離を修正した際）／**発見者:** Codex（レビュー）

### 症状

10秒以上読んだ直後にVaultを切り替えると、**旧Vaultのノートの読書痕跡が、切替後の新Vaultの `_ReadingTraces/` へ書き込まれる**。「これを防ぐための修正」を入れた直後にもかかわらず、防げていなかった。

### 再現経路

1. Vault A のノートを10秒以上読む
2. 別ノートへ移る（または背面化）→ `flush()` が保存コルーチンを起動
3. 保存が実際に走るまでの間に、オプションから Vault B へ切り替える
4. **Vault B の `_ReadingTraces/` に、Vault A のノートの相対パス・タイトルを持つJSONが増える**

### 根本原因 — 確認と使用で `vaultUri()` を2回読んでいた

「保存要求に、開いた時点のVault識別子を持たせて照合する」という対策自体は正しかった。壊れていたのは照合の書き方。
...
override fun write(key: String, bytes: ByteArray, vaultKey: String) {
    ...
    val current = folderIndex()                            // ← 2回目。ここで再び vaultUri() を読む
    ...
}
```

`folderIndex()` は保存先を「**その時点の** `vaultUri()`」から解決する。つまり1回目の確認を通過した後、2回目の読み取りまでにVaultが切り替わると、**確認したVaultと書き込むVaultが別物になる**。

`@Synchronized` を付けていたので守られているつもりだったが、これが誤り。`vaultUri` は `NoteViewModel` が持つ `var` で、**このクラスのロックの外**（メインスレッド）から更新される。`@Synchronized` が守るのは自身のフィールド（索引キャッシュ）だけで、外部の可変状態は一切守らない。

しかも `folderIndex()` は新しいVaultを見ると索引を作り直すので、**新Vaultに `_ReadingTraces` を作ってまで**書きに行く。窓が狭いどころか、対策前とほぼ同じ挙動に戻っていた。

### 修正

`vaultUri()` を読む場所を1箇所に集約し、**その1つの値を照合にも保存先の解決にも使う**。
...
private fun folderIndex(vaultKey: String): FolderIndex? {
    ...
    return folderIndexOf(vault)   // 照合したのと同じ vault を使う
}
```

こうすると、書き込み先は必ず「`vaultUri()` を読んだ瞬間に選択されていたVault」になり、それは照合で `vaultKey` と一致すると確認済みのVaultそのもの。読んだ直後に切り替わった場合は旧Vaultへ書き切るが、**それが正しい**（その訪問は旧Vaultのノートで起きたもので、永続化した権限も生きている）。

あわせて `NoteViewModel.vaultUri` へ `@Volatile` を付け、IOスレッドからの読み取りであることを明示した。

### 教訓

- **check-then-act は「確認した値」をそのまま使う。** 確認と使用で同じソースを読み直すと、その間の変化で別物を掴む。`if (current() == x) { use(current()) }` は常に疑う。正しい形は `val v = current(); if (v == x) { use(v) }`。
- **`@Synchronized` は自分の状態しか守らない。** ロックの外で更新される他オブジェクトの可変状態（ここでは ViewModel の `vaultUri`）は、同じロックに入っていないので一切保護されない。「ロックを付けたから安全」は、守る対象がロック内にある時だけ成立する。
- **「対策を入れた」ことと「対策が効いている」ことは別。** 今回は対策の方針（識別子を持ち回る）が正しかったぶん、コミットメッセージも設計書も「解消済み」と書けてしまった。効いているかは、**対策コードの中で外部状態を何回読んでいるか**まで見ないと分からない。
- **JVMテストの境界の外だった。** Fake Gateway は「不一致キーなら拒む」という契約は検証できるが、`vaultUri` が動く実物の解決経路は SAF 実装の中にあり再現できない。だから**構造で間違えられなくする**（読み取り箇所を1つにする）方が、テストを足すより確実だった。

---

## まとめ — AIが生成しがちな潜在バグの見つけ方

- 最新モデルが混入させる潜在バグには、少なくとも3つの型がある。**(A) 状態遷移型**（#1・#2）＝「別画面へ遷移して戻る」「折り畳む／展開する」を挟むとボタンが無反応になる。**(B) 応答解釈型**（#3）＝AIの出力が正常系から崩れると解釈が破綻する。**(C) 並行状態型**（#4）＝確認した値と使う値がズレ、稀なタイミングでだけ別の対象へ書く。混入者は #1 Claude Fable 5・#2 Claude Opus・#3 Codex・#4 Claude Opus と横断的。
- **共通DNA は「通常フローでは表面化しない」こと。** (A)は遷移を挟まなければ、(B)はモデルが素直に返せば、(C)はタイミングが重ならなければ、正常に見えてしまう。だから「通常フローで動く」を合格にしてはいけない。
- 検証の勘所：(A)は**遷移をまたいだ後**（クイズ→戻る／Fold開閉→戻る）に主要導線を1回叩く。(B)は**モデルが崩す代表パターン**（末尾記号・括弧・余計な語・前置き）をテストに入れ、防御的解釈を**全分岐で対称**にする。(C)は**外部の可変状態を何回読んでいるか**を数える。2回以上読んでいれば、その間に変わる前提で読む。
- **(C) には固有の危うさがある**: 対策を入れた本人（＝AI）が「解消済み」と書けてしまう点。方針が正しいと、実装が効いていなくても文書・コミットメッセージは通ってしまう。**修正の主張は、修正コードを別の目で読むまで確定させない**。
