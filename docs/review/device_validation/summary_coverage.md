...
| `COVER-05` | `RESULT` 行の生成msを並べる | 1回あたりの時間と合計を残す。**要約の保存（[note_summary](../../dev/features/note_summary.md) 判断6）で、開き直しのたびに消える待ち時間の見積もり**になる（→ [ai_input_budget](ai_input_budget.md) の計測と同じ扱い） |
