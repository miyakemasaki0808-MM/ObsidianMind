...
2. **重複起動のガードが誤って畳まない**（`MainActivity` が1枚のまま残り、ノートタブの見出し `Rediscover` が出ている）
...
| 本体（ノートタブ） | `Rediscover`（画面最上部の見出し）。`Vigilith AI` は出ない |

**ノートの名前を目印にしない。** 低い横画面では再会カードが本文の高さを奪い、アプリが閉じていなくても
ノートの名前が画面から消えることがある。起動の検証で見たいのはOPとガードなので、
カードに押し出されない見出しの `Rediscover` と、`MainActivity` の枚数で判定する。
本文が読めるかは、このケースの判定に含めない。
...
3. **監視テストを書く。** 共通手順の[一時テスト](quick_check.md#一時テスト)の規則に従い、
   ...
   **後処理で必ず削除する。** 次をすべて満たすこと。
   ...
   | 起動時のOPが終わり `Rediscover` を採取したら、ログへ `READY` を出す。**それ以降に `Vigilith AI` を1回でも採取したら、その場で失敗させる** | 合図の前に起きたOP（起動時の正常なOP）を不合格に数えない |
...
| ID | 入力・操作 | 期待 |
|---|---|---|
| `LAUNCH-05a` | **陽性対照。** 監視テストを起動し、`READY` が出るまで待つ（ユーザーは何もしない） | 起動時のOPの `Vigilith AI` を**1回以上**採取している。その後 `Rediscover` を採取し、`READY` が出る |
| `LAUNCH-05b` | `READY` を見てから、ユーザーへ「**横にしてください**」と合図する | 向きが横へ変わった記録がある。その変化に対応して `MainActivity` が**保存Bundle付きで**作り直されている。`Vigilith AI` を採取していない。作り直しの後に `Rediscover` が採取されている |
...
- **閉じたときにアプリがカバー画面へ続行されなかったら**（`Rediscover` が採取されない）、
...
| 各ケース | `LAUNCH-05b`〜`05e` ごとに、合図の時刻、構成または折りたたみ状態の前後、作り直しの回数と保存Bundleの有無、`Vigilith AI` の採取回数（0であること）、作り直し後に `Rediscover` を採取したか |
