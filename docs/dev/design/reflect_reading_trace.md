...
**関連:** [reflect_distill](reflect_distill.md)・[reflect_remark](reflect_remark.md)・[architecture](architecture.md)
...
ブロック番号だけでは、長大な段落・コード・表が1ブロックのノートで**冒頭しか見えていなくても100%**になる。

- **可視割合**は座標（Int）だけを受ける純関数 `visibleFractionOfBlock`（`ui/ReadingProgressGeometry.kt`）で求める
- **切り捨て**にする。丸めると末尾が残っていても100%になり、カードが「最後まで読んでいます」と誤断定する
- **報告は5%刻みへ量子化**してから `distinctUntilChanged` に掛ける（そのまま流すと毎フレーム報告になる）
- 最深判定は「index が深い、または同じ index でより深くまで見えた」。巻き戻しでは下がらない
- 高さが測れていないブロック（`size <= 0`）は全可視扱い（到達率を不当に下げないことを優先）
...
- **1痕跡=1ファイル**（`<sha256Hex(相対パス)>.json`）。相対パスは `/` を含みそのままファイル名にできない
...
- **documentId** は参考情報として持つが **checksum の対象に含めない**（再バインドで書き換わる値を整合性対象にしない）
...
長さ前置なので、区切り文字をタイトルに混ぜて別内容へ偽装できない。null と空文字は1バイトの存在フラグで区別する。

| 上限（バイト基準） | 値 |
|---|---|
| 訪問の保持 | 30件（超過時は古いものから捨てる） |
| AI要約 | 2,048バイト |
| 相対パス | 1,024バイト |
...
**上限は60秒に1回。** **まだ一度も読んでいないノートを開くと必ず不在になる**ため、
制限しないと開くたび全走査になり索引キャッシュを置いた意味が消える。
...
**旧Vault向けの要求は枠を消費しない**（切替直後の遅延要求1つで正当な作り直しが60秒止まる）。
