...
**状態:** 実装済み・稼働中（androidTest 16ファイル・99件）。CIでは実行せず、PR前に Android Studio で回す運用
**最終検証:** 2026-09-13
...
書式は端末のモデル世代や更新で変わるので、assert すると**アプリを変えていないのにテストが赤くなる**。
同じモデルのうちは出力が決定的でも（→ [ai_quality_measurement](ai_quality_measurement.md) 判断8）、書式を固定してよい理由にはならない。
