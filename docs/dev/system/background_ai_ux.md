...
**`Error` variant を持つのは、失敗が実際に起きるほうだけ。**
`PickerResult.Error` は**生成の例外から実際に構築される**（`SearchPickerUseCase` の
広い catch）。関連ノートは例外でも決定的チャンネルを返すので、**結果にも UI 状態にも
失敗の variant を持たない**。冊子の「編めない理由」も「探している最中」と「見つからなかった」の2つになる。
...
### 判断6: 理由をボタンのラベルへ潰さない（2026-08-15 の実機レビュー）

**机上の4巡では出ず、実機で出た。** セクション要約が `Ready` で成功したあと、
クイズの状態確認だけが `Unsupported` を返す順序で再現する。
シートは `QuizState.AiNotice` を「クイズを使えません」というラベル1つへ潰し、
恒久非対応ではボタンも無効にしていた。**理由文を描くのは `QuizScreen` だが、
それを開く手はそのボタンしか無い** — 押した本人に、使えない理由も次の行動も残らなかった。
...
**要約は、英文を出さず開き直しを促す**（オーナー判断 2026-09-17 → [note_summary](../features/note_summary.md) 判断9）。
自動機能は黙って劣化させ、例外本文をユーザーへ出さない（判断1・判断4）という方針から、
**黙るほうではなく「例外本文を出さない」ほうを取った** — 要約欄ごと消すと次の行動が残らず、
断られたときは少し待てば通りうるので、開き直しという行動を返せる。
判定は `ai/AiGenerationFailure.kt` の `isAiCoreBusy` が持つ。**要約以外の経路は文言を変えていない**
（分野判定と痕跡の要約は元から黙って諦める。押して使う機能は例外の文言のまま出る）。
回数制限そのものの露出は、要約の保存（→ [note_summary](../features/note_summary.md) 判断6）で
開き直したノートの生成が消えたぶん下がる。
**アプリ本体でノートを素早く開き続けたときの冊数・秒数は測らない**（オーナー判断 2026-09-14）。
計測テストの条件で観測した境界（12回成功後、13回目で拒否）で足り、窓の長さと同じくアプリから変えられないため。
