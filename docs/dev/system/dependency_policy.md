...
**対象領域:** `app/build.gradle.kts` の依存宣言・`gradle/wrapper`・Lintの更新系3チェック・**推移依存が持ち込む権限**
...
> **件数を数えるときは `--rerun-tasks` を付ける（2026-09-12）。**
> `lintAnalyzeDebug` が UP-TO-DATE だと前回のXMLがそのまま残るので、
> **解析を走らせずにレポートを開くと、実際とは違う件数を読む。**
> 実際この日、残っていた空のレポートを見て「`--offline` では0件になる」と誤認し、
> **正しかった記述を誤りとして4箇所へ書き換えた**（本節・CLAUDE.md・`build.gradle.kts`・課題台帳）。
> **フラグから件数を説明しない。タスクが実行されたことを確認して数える。**
...
**ただしセキュリティ修正が出たときだけは区切りを待たない。**
現在の12件について「セキュリティ修正を含むか」は一度も見ておらず、
**距離（版がいくつ離れているか）しか測っていない**。距離は放置してよいが、修正は放置の可否が違う。
...
## 判断5: 依存が持ち込む権限は、宣言しないだけでなく成果物で数える

**状態: 既存モデルでの実機確認済み。モデルDLは確認待ち**（2026-09-12、Pixel 10 Pro Fold / Android 17）。
**確認は契機待ちにしてある** — 確かめるために未DL状態を作りにはいかない。

ADR-0002 は「ネットワーク権限を宣言しない」と書いてクラウド併用を退けた。
だが**宣言しないことと、成果物に無いことは別だった**。

| | 出どころ | 混ざっていたもの |
|---|---|---|
| マージ後マニフェスト（debug・release とも） | `transport-backend-cct:2.3.3` | `INTERNET`・`ACCESS_NETWORK_STATE` |

`INTERNET` は通常権限なのでインストール時に黙って付与され、端末の設定画面には「インターネット」と出る。
**保証がソースにだけあり、端末に入るAPKには無い**状態だった。

### 依存からは外せない。マニフェストで除く

`transport-backend-cct` は `genai-prompt` の直下と `mlkit:common` 経由の**二重**で入る。
`exclude` するとクラスごと消えて実行時に落ちるので、**`tools:node="remove"` で権限だけを除く**。

**除いて安全と見た根拠。** モデルDLと生成は AICore アプリ（別プロセス）が行うのでこちら側の権限は要らない。
計測の送信経路は権限を失うと `ConnectivityManager` の呼び出しで失敗するが、
`Uploader` は `SafeLoggingExecutor` 上で走り、**例外はログに落ちてアプリまで伝わらない**
（beta2 のバイトコードの例外表で確認）。

**既存モデルでは実機確認済み。** 9機能に対応する13プロンプトの生成、前面5分→背面5分の連続観測、機内モードの3経路が成功した。実機では `WorkInitializer` からのジョブ登録が権限不足で失敗し、`SafeLoggingExecutor` が例外をログ化した後も同一プロセスで生成と観測を完走した。これは `Uploader` 内の通信状態取得とは別の経路である。**モデル未DLの端末が無いため、モデルDLの確認だけは残る。**

### 検査は増減の両方を見る

`verify<Variant>ManifestPermissions` が、マージ後マニフェストの `uses-permission` を
**期待する集合と双方向で突き合わせる**。

- **増えた側**を見るのが本体である。今回の2件を名指しで禁じるだけでは、
  **次に別の権限を持ち込む依存**を素通しする。危ないのは混ざったことではなく、
  **混ざったのに誰も気づけなかったこと**のほうだった
- **減った側**も見る。`tools:node="remove"` が AICore への接続権限まで巻き込む事故は、
  ビルドは通り、実機でAIが丸ごと黙る形で出る

### この検査が見ていないもの

**端末が足す分は見えない。** 実機で旧版の要求権限を数えると**5件**あり、
マージ後マニフェストの4件より `ACCESS_LOCAL_NETWORK` が1件多かった。
マニフェストのどこにも無いので、**インストール時にOSが足している**
（`INTERNET` に伴うものと見られるが、機序は確かめていない）。新版では3件とも消えたので実害は無い。

**が、「成果物で数える」はインストール前までしか届かない**ことの実例である。
端末に入った後の要求権限は実機ケース `PERM-01` が見る。
**机上の検査と実機ケースは、同じものを二重に見ているのではない。**

**JVMテストにはできない。** 見たいのはソースではなくビルド出力で、
`app/build/intermediates/...` はJVMテストの入力として宣言できず、
`testDebugUnitTest` 単独では生成すらされない。

**走る契機はdebugとreleaseで違う。** debug は `lintDebug`・`assembleDebug*` が
`processDebugMainManifest` を通るので素通りできない。
**release はCIでビルドしないので契機が来ない**ため、`ci.yml` が名指しで呼ぶ。
これが、**未署名で実機に入れられない release 成果物の権限を保証する唯一の経路**である。

**期待値を動かすときは、`app/build.gradle.kts` の `expected` とこの節を同時に直す。**
片方だけ直すと、検査は通るのに根拠が消える。
