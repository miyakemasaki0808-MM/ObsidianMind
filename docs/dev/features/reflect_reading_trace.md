...
### 判断: 返事の預け先は、要求を出した時点のセッションで決まる（2026-09-10）

**保存先の `vaultKey` は開始時に固定していたが、失敗側の預け先だけが現在値を読んでいた。**
`saveReply` の読込は同期I/Oなので、戻る頃には別のノートを開いていることがある。
そのとき `holdOrLose` が「いまのセッション」へ預け直すと、
**Aへ書いた返事がBのひとことと組になり、Bのサイドカーへ保存される。**

- **所有者（`Session.id`）を非同期へ入る前に固定し、預ける前に照合する。**
- **所有者が違えば預けず `Lost` を返す。** 別のノートへ混ぜて「保存できた」ことにするより、
  保存できなかったと言うほうが正直である。
- **これは「成功側だけを固定した」型である**（→ [lessons](../lessons.md) L26）。
  非同期の要求は、**失敗側の後始末でも同じ所有者を使う**。
...
#### 判断18: 取り消せない操作は、割り込ませずに1列に並べる（2026-09-18）

洗い出しと削除が**同じJobを共有**していたため、候補を2件続けて消すと後のタップが前の削除を
キャンセルしていた。SAFの `deleteDocument()` は同期I/Oで、**キャンセルしても物理削除は巻き戻らず、
状態更新だけが落ちる** — 消えたのに画面へ残る、という見え方になっていた。

**キャンセルで捌けるのは、やり直しが効く処理だけである。** 生成や走査は捨ててやり直せるが、
外部への削除は取り消せない。**この区別がチャネルではなく操作の性質から来る**ので、
世代照合やキャンセルの規約（→ [architecture](../system/architecture.md) 判断4）とは別に要る。

決めたことは4つ。

- **洗い出しと削除でJobを分ける。** 削除を止めてよいのは**Vault切替のときだけ**で、
  そこでは一覧ごと捨てるので更新が落ちても矛盾が残らない
- **両方を1つの `Mutex` で1列に流す。** 錠が守っているのは結果ではなく**外部I/Oの本数**である
  （結果のほうは下の「最新へ当てる」で合う）。**外すと、候補を続けてタップした数だけ
  Vault全走査が同時に走る** — 遠いプロバイダではそれ自体が体感になる
- **結果は押した時点の一覧ではなく、最新の状態へ当てる。** 捕捉した一覧へ当てると、
  **後から終わった削除が、先に終わった削除の結果を取り消す**
- **走行中の候補はボタンを止める**（`deletingKeys`）。始まった削除は後から止められないので、
  二重に走らせないことを手前で防ぐ

**失敗と未確認は件数ではなくキーで持つ。** 件数だと、別の候補を消せたときに
**まだ一覧に残っている候補についての報せだけが消える。** キーなら、その候補を消し直せたときにだけ消える。

**錠は変異で確かめてから残した。** 外しても結果は合うため最初はどのテストも落ちず、
「落ちるテストを書けないガードは削除の候補」（→ [lessons L11](../lessons.md#l11-テストが効いているかは変異させて確かめる)）に
当たっていた。**守っている対象が結果ではなくI/Oの本数だと分かったので、
走査と削除の順序を固定するテストに置き換えた。**
