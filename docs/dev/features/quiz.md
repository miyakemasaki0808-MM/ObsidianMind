...
- **保証していないこと:**
  ...
  - **設問が毎回同じとは保証しない。** 生成は決定的なので、同じ抜粋からは同じ設問が返るはずだが
    （→ [ai_quality_measurement](../system/ai_quality_measurement.md) 判断8）、実機で確かめたのは要約だけで、
    端末のモデルが更新されれば設問も変わる
...
| キャッシュが無い | 同じノートで押し直すと再生成になる。**返るのはおそらく同じ設問で、払うのは待ち時間だけ** |
