...
> **通しで読みたいときは [document_map.md §5 の通読ルート](document_map.md#通しで読む--設計思想を掴む順)。**
> `features/` `system/` のファイル名に番号は付けない。**普段は読む順ではなく引く順で使う**ためで、
> 順路が要るのは「設計思想を通しで掴む」ときだけなので、順路のほうを索引が持つ。
