...
補助として、単発の潜在バグを記録する [bugfix_reports.md](bugfix_reports.md) と、出発点の記録 [project_origin.md](../owner/project_origin.md) がある。
...
「なぜその設計にしたか」を機能単位で残す。**さらなるフォルダ分けはしていない**（一覧で足りるため。読みにくくなったら下の4分類でサブフォルダ化する）。

> **サブフォルダ化の契機（20本）には既に達している。** ただし移動すると各文書の相対リンクと
...
>
> **例外は [`review/2026-*.md`](../review/README.md) のレビュー成果物のみ。**
> レビュアーが書くスナップショットなので、こちらから手を入れない。
