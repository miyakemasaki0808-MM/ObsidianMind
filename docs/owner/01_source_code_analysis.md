...
**測定日:** 2026-09-20。統計は同日に現行ソースから再測定した。基準は `77b29d0` で、作業ツリーは clean

## 目次

- [0. 規模の推移](#0-規模の推移)
- [0.1 検証状態（2026-09-20 時点）](#01-検証状態2026-09-20-時点)
...
- [13. テスト状況](#13-テスト状況)
  ...
  - [13.5 instrumentation の内訳（106件）](#135-instrumentation-の内訳106件)
...
| 指標 | 前回 2026-09-19 | 今回 2026-09-20 | 増減 |
|---|---:|---:|---:|
| 本番コード（ファイル） | 167 | **166** | -1 |
| 本番コード（行） | 29,780 | **29,217** | -563 |
| JVMテスト（ファイル） | 143 | **142** | -1 |
| JVMテスト（行） | 31,297 | **30,679** | -618 |
| JVMテスト（件数） | 1,556 | **1,508** | -48 |
| instrumentation（件数） | 101 | **106** | +5 |
| debug ソースセット（ファイル） | 4 | **4** | ±0 |

行数は空行・コメントを含む `wc -l` で、生成物と Gradle スクリプトは含まない。テスト件数は `@Test` の出現数。
本番の29,217行の内訳はコメント 8,123・空行 2,091・本体 19,003で、コメントの比率は28%である。
...
**今回は初めて減った。** 前回の測定以降の12コミットのうち `app/src` に触れたのは7件で、
「ノートへのひとこと」の撤去が本番410行・JVMテスト169行を落とし、置き換えた「余白メモ」がそれを埋め切らなかった。
**機能が1つ減って1つ増えたのではなく、AIを1本畳んで、AIを呼ばない口へ置き換えた。**

1. **ひとことの撤去。** 生成（`RemarkController`）・専用画面（`RemarkScreen`）・応答検証（`RemarkComposer`）・
   AIタブの入口・Snackbar通知を経路ごと外した。専用ルート `remark` も無くなり、全画面ルートは7つから6つになった。
   **設計の記録は残してある** — なぜ作り直し、なぜ畳んだかは [reflect_remark](../dev/features/reflect_remark.md) が
   廃止済みの判断として持つ。§6.8
2. **余白メモの新設。** 読んでいる最中に、何にも紐づかない短い断片を何度でも置ける口。
   `MarginMemoController`・`MarginMemoComposer`・`MarginMemoState`・`MarginMemoSheet` の4本が新規で、
   **Reflect 系で唯一、端末AIを1回も呼ばない。** ボトムシート1枚で完結するのでルートは増えていない。
   JVMテストは Controller 16件・入力整形8件が増え、ひとこと側の63件が消えた。§6.8
3. **痕跡サイドカーを schema v7 へ上げた。** ひとこと・返事・映し返しの3欄を読み捨て、
   メモの配列を置いた。v1〜v6 からの読み込みは引き続き通る。§6.11
4. **実機で触る口をUIテストへ置いた。** `MarginMemoSheetUiTest` の4件で instrumentation が増えている。
   残り1件は前回値の数え落としで、`DistillRangeAdjustUiTest` は当時すでに9件あった。§13.5

## 0.1 検証状態（2026-09-20 時点）

| | 結果 |
|---|---|
| `testDebugUnitTest` | **1,508ケース全件グリーン。** 133テストクラス・failure 0・skip 0・3秒 |
...
| instrumentation 実行 | **全106件を一度に通した実行は無い。** 直近は機能単位で、09-13 に要約の基準線9ケースと全27組、09-12 に分野色・権限除去後の生成と連続10分の観測と機内モード。いずれも Pixel 10 Pro Fold・Android 17。**この6日間に増えた余白メモの4件と、範囲調整の9件のうち自由範囲ぶんは、まだ実機で走らせていない** |
| 実機検証 | 09-20 に蒸留の自由範囲 `DIST-25`〜`DIST-31` を通した。22ファイルの実保存が指定範囲どおりで、元Vaultのハッシュも不変だった。09-18 には要約の保存7ケースと自動生成の門番を通している |

**実機確認が済んでいないもの:** **余白メモの全ケース**（机上ゲートは通っているが、実機と突き合わせた日はまだ無い）、
ひとことの導線が残っていないことの確認、ランチャー再タップの起動ガードのうち回転と Fold 開閉の1ケース、
権限除去後のモデルDL、Vigilith Phase 3 の目視。モデルDLは契機待ちで、確かめるために未DL状態を作りにはいかない。
**未対応の課題は [_wip/current_issues.md](../_wip/current_issues.md) が正本**で、現在は高4件・中8件・低5件・超低2件。

> **高が0件から4件へ増えたのは、実装が悪化したからではない。** 余白メモは机上ゲートを通した直後の状態で、
> 実装レビュー8件・修正レビュー4件で見つかった欠陥がそのまま台帳に載っている。
> **実機検証が済めばその場で消える行**であり、いま台帳が重いのは**検証待ちの列が長いこと**を示している。
...
> **instrumentation が106件あることは、保証範囲が106件ぶん広いことを意味しない。**
> 主張が実際に試していることより広い箇所が3件ある。`ActivityScenario.recreate()` はプロセス死亡を覆わない、
> 端末AI生成は11本のプロンプトのうち5本、連打の主張は撤回済み。→ §13.6
>
...
非AIの補助機能を3つ持つ。**余白メモ**は、読んでいる最中に浮かんだ短い断片を、本文に触れずに何度でも置ける口である。端末AIを1回も呼ばないので、Nano非対応の端末でも同じに動く（§6.8）。**読書痕跡の書き出しと読み戻し**（データ管理画面）では、訪問記録・AI要約・**ユーザーが書いた余白メモ**を1ファイルへ退避でき、Vaultのフォルダが消えても読み戻せる（§6.12）。**冊子モード**（2026-08-30〜09-02）はランダムを1回押して10枚の束を作り、上へ繰る独立ルートで、各ページには本文から**抽出した**代表文1行（扉）が出る。**冊子ではAIも訪問記録も走らせない** — 記録と生成が始まるのは「これを読む」で通常表示へ渡ったときだけである（§6.13）。冊子の紙は通常表示と**面の形で**分けてあり、断ち切った紙の角・四辺の余白・背後に控える紙の縁を持つ。

Q&Aはバックグラウンド生成方式で、生成中もノート閲覧を継続でき、完了・エラーはSnackbarで通知される。**タブのバッジはもう無い** — 駆動源がひとことの生成中だけだったので、ひとことの撤去で対象ごと消えた。ReadingTraceは対照的に、AI未準備・生成失敗を通知せず、生の痕跡だけを先に表示して黙って劣化する。AI以外の補助機能として、当日分のみの閲覧履歴（さがすタブ「今日読んだノート」）を持つ。

アーキテクチャは「単一 Activity + Compose Navigation + 単一 ViewModel」を入口としつつ、肥大化を避けるため要約・検索・セクションチャット・クイズ・余白メモ・旧補記ファイルの片付け・蒸留・読書痕跡・再会カード・本文セクション解析・痕跡の整理／退避・冊子・分野判定を機能別 Controller（**14個**）に分割している。`NoteViewModel` は `Uri`・`ContentResolver`・`SharedPreferences` を扱うAndroid境界だけを担い、Controller間の調停と状態所有は Android API を呼ばない `NoteSessionCoordinator` が持つ。依存の組み立ては `NoteViewModelDependencies` へ外出しされている。ファイルI/Oは `NoteRepository`、AI判定を含む主要ロジックは UseCase、AI接続は `AiClient`、Markdown生成・応答パースは純粋ロジックへ分離されている。蒸留のVault書き戻しは `DistillWriteRepository` が専用の安全書き込み経路（ハッシュ照合・復旧レコード）を持ち、ReadingTraceは `_ReadingTraces` へのベストエフォートなサイドカー保存を持つ。**余白メモもこのサイドカー（schema v7）へ入る** — ノート本文の `.md` へ書くのは蒸留の太字化だけである。

現時点の総評は次のとおり。

- 主要責務の分割、状態の一元管理、古いAI処理のキャンセル、生成タイムアウト、SAF走査キャッシュが実装され、継続的な機能追加に耐えやすい構造になっている。
- Markdownパーサー、**余白メモの入力整形**、クイズ応答パーサー、蒸留の文分割・採点・太字挿入・**範囲プリセットと自由範囲のスナップと重なり解消**、ReadingTraceのJSON・Controller・相対パス走査、**冊子の扉の抽出規則**、Vigilith起動・表示状態・状態別モーション・配置計算、明暗トークンのコントラストなど、壊れやすい純粋ロジックにはユニットテストが整備されている（**1,508ケース**。内訳は §13.1）。
...
- **SAF・画像復号・Compose描画・画面遷移・画素・端末AI生成を実機で通す instrumentation が 106件そろっている**（→ §13.5）。土台は `src/debug` のテスト用 `DocumentsProvider` で、本番の `NoteRepository` / `SafVaultBrowser` をそのまま動かす。**ただし保証範囲は主張より狭い** — 連打の主張は撤回済み、`recreate()` はプロセス死亡を覆わず、端末AI生成は11本のプロンプトのうち5本だけである（→ §13.6）。**CIでは実行しない**判断を 2026-08-08 に確定した（→ §13.3）。
...
- 構造面の成長限界（依存の循環・ViewModelのテスト不能・状態の共有所有）は 2026-07-27 のB案で解消した。アクセシビリティとリリース構成は 2026-07-29〜30 のD案・E案で着手し、**ライトの文字トークンは実際に載る面すべてで4.5:1を満たす**ようになった。**下部ナビ帯の上でコントラストを取れなかったバッジ塗りは、2026-08-09 のひとことの作り直しで未確認管理ごと不要になり、2026-09-20 のひとことの撤去でバッジそのものが消えた。**残る弱点は R8・署名が未設定であること、そして **instrumentation の実行がCIで担保されず「PR前に手で回す」運用のままであること**に移っている。**ユーザーが書いた言葉の退避手段が無い**という弱点は、2026-08-23〜28 の書き出し／読み戻し（§6.12）で解消した。退避の対象は、いまは返事ではなく余白メモである。**「構造では分けたのに画面がその分離を見せていない」という弱点は、2026-09-02 の紙面の形（§6.13）で解消した。** ただし**そこで露わになった穴のほうは残っている** — 振る舞いの検査が全緑でも佇まいは判定されない。テストで埋める種類ではないので、実機検証のケース表が引き受ける形にした。**機能の看板に掲げた狙いにチャネルの持ち主がいない**という別の穴は、動きの行を二分して手触り側を冊子へ渡すことで閉じた。**AIの品質を測る手段が無い**という弱点は、2026-09-13 に要約の採点器と固定コーパスが入って半分埋まった。語の重なりしか測らず意味は見ないので、残り半分は実機の出力を人が読む工程が持つ。
...
| 区分 | ファイル数 | 行数・件数 |
|---|---:|---:|
| 本番 Kotlin | 166ファイル | 29,217行。うちコメント 8,123・空行 2,091・本体 19,003 |
| ユニットテスト Kotlin | 142ファイル | 30,679行、1,508テスト。テストクラスは133で、残り9つは共有フェイク・共有ヘルパ |
| instrumentation テスト Kotlin | 17ファイル | 4,422行、106テスト。**全件を一度に通した実行は無い。** 直近は機能単位で実行している。内訳は §13.5 |
...
│   │   │   └── PromptBuilder.kt                # 各機能のプロンプト構築（11本）
│   │   ├── controller/
...
│   │   │   ├── MarginMemoController.kt         # 余白メモの読み出し・追記・削除（**AIを呼ばない唯一のController**）
...
│   │   │   ├── MarginMemoComposer.kt           # 余白メモの入力整形（制御文字の除去・上限での切りと通知）
...
│   │   │   ├── ReadingTrace.kt                 # 読書痕跡モデル・上限・検証・余白メモの配列（schema v7）
│   │   │   ├── ReunionKind.kt                  # 再会カードの枠に出ている1件の種別
│   │   │   └── state/                          # 機能別の sealed state（Note/Summary/RelatedNotes/Search/
│   │   │                                       #   Quiz/MarginMemo/AnnotationList/Distill/DistillRangeEdit/
│   │   │                                       #   SectionChat/ReadingTraceCard/ReadingTraceCleanup/
│   │   │                                       #   ReadingTraceBackup/Booklet/AiStatusNotice）
│   │   └── ui/
│   │       ├── AppScaffold.kt                  # 5タブ、NavigationBar/Rail切替、SnackbarHost
...
│   │       │   ├── AiTab.kt                    # 要約の表示と、蒸留（Reflect）の起点
...
│   │       │   ├── MarginMemoSheet.kt          # 余白メモのボトムシート（入力欄とこのノートのメモ一覧）
...
├── test/java/com/example/newproject/           # 142ファイル・1,508テスト（内訳は §13.1）
...
├── androidTest/java/com/example/newproject/    # 17ファイル・106テスト（内訳は §13.5）
...
│   ├── ai/OnDeviceGenerationTest.kt            # 本番プロンプトでの実生成（11本中5本）
...
│   ├── ui/MarginMemoSheetUiTest.kt             # 余白メモのシート（入力・一覧・削除の確認・保存中の保持）
...
Compose UI / MainActivity
     ...
     NoteSessionCoordinator               ← 横断調停・状態所有（Android API を呼ばない）
       ...
       ├── QuizController
       ├── MarginMemoController                 ← **AIを呼ばない唯一のController**
       ├── AnnotationController                 ← Vault単位
       ├── DistillController
       ├── NoteSectionController
       ├── ReadingTraceController
       ├── ReunionCardController                ← 痕跡の錠を ReadingTraceController と共有
...
MarginMemoComposer / NoteTitleNormalizer / AiResponseParsing / AiStatusNotices /
...
| `ReadingTraceController` | `ReadingTraceStateWriter` | 読書中Session自体はController内部。**UI状態を書くのは再会カード側だけ**で、こちらは書かない |
| `ReunionCardController` | `ReadingTraceStateWriter` | `readingTraceCard`（`ReadingTraceController` と**同じWriterと同じ錠**を受け取る） |
...
- `marginMemoState`: Idle / Loading / Ready / Error。`Ready` はこのノートのメモ一覧と、保存の状態（None / Saving / Held / Saved / Failed / Full）と、切り詰めたかどうかを持つ。**保存の状態を真偽の束にしない** — 「保存中かつ未保存」のような無意味な組み合わせを型として作れてしまう
- `isMarginMemoSheetVisible`: 余白メモのシートの表示有無。セクションチャットのシートと同じ形で、ノート切替では**一覧と一緒に必ず落とす**
...
`quizState` の `sourceTitle` は「どのノートの生成結果か」を表し、Snackbar通知の判定に使う。`isViewed`（未確認管理）を持つのも `quizState` だけである。通知の発火判定キーは `toEventKey()` 拡張関数（`model/state/`）が組み立てる。

各 sealed state は `model/state/` 配下の機能別ファイルに分かれ、集約する `NoteUiState`（23フィールド）だけが `model/NoteUiState.kt` にある。

ノートまたはVaultの切替時は `NoteUiStateStore` の `withNoteScopedReset()` により、紙の地色・要約・関連・クイズ・余白メモ（一覧とシートの可視）・セクションチャット・再会カードを一括リセットする。Vault切替時だけ `withVaultScopedReset()` が追加で走り、検索スコープ・閲覧履歴・旧補記一覧・痕跡の整理／退避・**冊子の束**・**分野の索引**を落とす。**冊子をノート単位に登録しないのは機能の目的そのもの** — 「これを読む」でノートへ渡って戻れば同じ10枚が同じページで残ることが冊子の定義だからである。`noteState` と `wikilinkTitles` はVault切替でも残す — 切替直後に `loadRandomNote` が走って差し替わるため、ここで落とすと画面が一瞬空白になるだけだからである。
...
`AppScaffold` は `WindowSizeClass` を参照し、Expanded幅では左側 `NavigationRail`、それ以外では下部 `NavigationBar` を使用する。選択タブのインジケータは `Aqua`（Indigo地に埋もれないアクセント）。全画面ノート・冊子・クイズ・データ管理・痕跡の整理・旧補記管理の全画面ルートではタブUIを表示しない。全画面ノートは進入中にシステムバー（ナビ＋ステータス）も隠し、離脱時にナビバーのみ復元する（ステータスバーはアプリ全体仕様どおり隠したまま）。
...
- 本文パネルのフェード＋スケール表示
- ヘッダ右の **✎** で余白メモのボトムシートが上がる。本文はスクロール位置ごと背後に残り、置いてもシートは閉じない（§6.8）
...
- スクロール位置から現在セクションを判定
- 最終可視Markdownブロックとその可視割合をReadingTraceへ報告し、Rediscover由来で過去痕跡があれば本文上に「前回のあなた」カードを表示。メモが1件以上あるとカードに「前回のメモを見る」が出て、同じシートが開く
...
- モデルダウンロード時は進捗を表示
- **タブアイコンのバッジは無い。** 唯一の駆動源だったひとことの生成が 2026-09-20 に無くなり、判定関数ごと削除した
...
Q&Aの生成開始／完了／失敗は `MainActivity` の `LaunchedEffect` がSnackbarで通知する。完了・失敗の通知にはアクション（見る／詳細／もう一度）が付き、タップで結果画面を開く。表示済みイベントキーを `rememberSaveable` に記録し、画面回転による再表示を抑止する。**余白メモは通知を持たない** — 置いた結果はその場のシートに増えるので、画面の外から知らせる必要がない。全画面ノート（`note_fullscreen`）表示中はSnackbarを抑制し、AIの状態は全画面の最小FABが担う。
...
**2026-09-20 に「ノートへのひとこと」を撤去し、ここへ置き換えた。**
ひとことは**AIが問いを投げ、ユーザーが答える**形で、Reflect のなかでユーザーへの要求が最も重かった。
しかも1ノート1件・上書きなので溜まらない。余白メモはその要求を全部外す — **問いを出さず、答えさせず、溜める。**
正本は [reflect_margin_memo](../dev/features/reflect_margin_memo.md)、
畳んだ側の判断は [reflect_remark](../dev/features/reflect_remark.md) が廃止済みの記録として持つ。

**AIを1回も呼ばない。** Reflect 系で唯一、端末AIの可否と無関係に動く機能である。
Nano非対応の端末でも同じに動き、AI状態の案内も出さない。

**入口と流れ**

1. ノート本文ヘッダ右の **`✎`** を押すと、ボトムシートが上がる。**本文はスクロール位置ごと背後に残る。**
2. シート上段が入力欄、下段がこのノートの既存メモで、**新しい順**に並ぶ。
3. 書いて `置く` を押すと一覧の先頭に増え、入力欄が空になる。**シートは閉じない**ので続けて書ける。
4. 各行の `消す` は確認ダイアログを挟む。**編集は無い** — 直したければ消して書き直す。
5. 後日 Rediscover で同じノートを引き当て、メモが1件以上あれば、再会カードの行が
   **`前回のメモを見る`** になり、押すと同じシートが開く。

**ルートを増やしていない。** ひとことは結果が長く待ち時間もあったので専用ルート `remark` を持っていたが、
余白メモはボトムシート1枚で完結する。全画面表示中は書けず、`✕` で通常表示へ戻ってから置く。
...
| 静かに知らせる目安 | 200字。**壁ではなく合図**で、切りも拒否もしない |
| 1ノートの保持件数 | 20件。**古いものから捨てない** |
| 書いた場所（見出し名） | 512バイト。訪問と同じ枠を使う |
| 痕跡ファイル全体 | 128KB |

**受け取ってから切り、切ったら必ず示す。** 200字を超えたら合図だけ、1,024バイトで初めて切り、
「ここまでを置きました」と伝える。20件に達したら置けないが、**入力欄の文字は消さない** —
消すと「置けなかった」と「消された」が同じ顔になる。
上限どうしの整合は計算ではなく**実シリアライザで測って**固定してある。
JSONは `"` と `\` を2バイトへ、制御文字を6バイトへ広げるので、
各欄の上限を満たしたままファイル上限を超えられるためである。
入力は保存前に**改行とタブ以外の制御文字を落とす**ので、JSON化後の膨張が最悪2倍に収まる。

**保存（`MarginMemoController` → `ReadingTraceController`）**

`_ReadingTraces/*.json`（schema v7）の `memos: List<MarginMemo>` へ追記する。**Vaultの `.md` は触らない。**
1件は本文・書いた日時・**置いたときに見ていた見出し**の3つを持つ。見出しは歴史的記録で、改名しても解決し直さない。

- **保存結果は3値で持つ。** `Saved` / `Held`（預かった）/ `Failed`。
  痕跡ファイルは離脱・背面化でしか作られないので、初読では `Held` になる。
  **`Held` を「保存済み」と呼ばない。**
- **`Full` を `Failed` に畳まない。** 再試行で直るものと、消さなければ直らないものは次の行動が違う。
- **切り詰めたことは保存の成否と別に持つ。** 切り詰めたうえで保存は成功しうる。
- **保存と削除は同じ錠で直列化する。** どちらかが走っている間にもう一方が走ると、
  互いの結果を取り消しうるため。
- **訪問の保存で容量が足りないときは、メモに一切触らず訪問だけ書く。**
  切り詰めて書くと、別端末や読み戻しで入った保存済みのメモを黙って捨てることになる。

**入力欄の中身は画面側が持ち、受け取れたと分かってから空にする。**
状態には受理の通算件数だけを置き、その件数が増えたときに空にする。
状態から「置けたら空にする」を導くと、`status` が保存後も残るため
**再コンポーズのたびに条件が成立し、2件目に書いた文字が入力するそばから消える。**

**ひとことの3欄は v7 で読み捨てる。**
`remark` / `remarkedAt` / `reply` / `repliedAt` / `mirrored` はモデルへ載せない。
ただし**checksum は書かれた版の正規形で検証してから落とす。**
先に落として照合すると全ファイルが不一致＝破損扱いになり、
破損した痕跡は次の訪問が新規として上書きするので、**訪問履歴まで道連れになる。**
しかも旧欄は同時に入ったのではなく、v3・v4・v5 と1つずつ足されている。
**null も存在フラグを1バイト書く**ので、窓を一律にすると守るつもりの訪問履歴を失う。

**旧補記ファイルは消さない。** 2026-08-09 より前に生成された `_AI補記/*.md` はVaultに残るので、
...
7. 保存後は `openNote()` を使わず**本文専用リロード**（`reloadNoteBody`／`withDistillBodyReloaded`）：要約・関連・余白メモは維持し、生Markdown文脈に結び付くセクションチャット・クイズは破棄する。`DistillController` が requestId で全フローを直列化し、ノート切替でキャンセルする。
...
**2026-09-20 に `DIST-25`〜`DIST-31` で実機確認した。** 22ファイルの実保存が指定範囲どおりで原文とバイト一致し、
元Vaultのハッシュも不変だった。**指で狙ったときの掴みやすさと縦送りの手触りは測っていない** —
検証の入力は座標指定なので、人の指の成功率とは別物として数える。
...
**持ち主は2つに分かれている**（2026-09-15）。訪問の記録と余白メモの保存は `ReadingTraceController`、再会カードの照合・AI要約・印は `ReunionCardController` が持つ。**UI状態を書くのは後者だけ**で、両者はサイドカーの read-modify-write を直列化する錠を共有する。
...
痕跡サイドカーは Vault 内の `_ReadingTraces/` にあり、フォルダごと消えれば
**再生成できない文章、つまりユーザーが書いた余白メモ**まで失われる。
データ管理画面から1ファイルへ書き出し、そこから読み戻せるようにした。
...
- **メモは配列なので、欄ごとの突き合わせで合流できる。** 1ノート1件・上書きだった返事では、
  端末側と退避側のどちらかを捨てるしかなかった。
- **合流後に20件を超えるノートは、無変更のまま保留する。** 端末側を書き換えず、退避側も捨てず、成功として数えない。
  退避ファイル内で同じノートが重複していて畳めないときも、そのパスごと保留する。
...
- 旧補記フォルダの列挙は `queryChildren()`。**書き出す経路はもう無い。**
...
`generate()` は companion object の `Mutex` で直列化される。要約、関連推薦、検索、クイズ、セクションAI、ReadingTrace俯瞰要約、分野判定が同時に要求されても、モデル生成は1件ずつ実行される。
...
**出力トークン上限の制約（genai-prompt 1.0.0-beta2）**: `GenerateContentRequest` の `maxOutputTokens` は1〜256しか受け付けず、超過値は `IllegalArgumentException` で全生成が失敗する（実機で確認済み）。このため上限は明示設定せずSDKデフォルトのまま運用し、各機能のプロンプト側で「256トークン程度に収まる出力要求」に絞る方針をとる（クイズ2問固定・分野判定はID1つ、など）。**この枠がゼロサムであることは設計判断の材料として残る** — 旧「AI補記メモ」は分類ラベルを同時に出させて本命の側を痩せさせており、その教訓は機能が消えた後も [reflect_remark](../dev/features/reflect_remark.md) が持っている。
...
- 上表の本文を持つ経路は、`PromptBuilder` ではなく呼び出し側が `buildNoteExcerpt(content, 上限)` で `NoteExcerpt` を作って渡す。`PromptBuilder` に `take()` は残っていない。
...
**世代は二層になっている。** ノート単位の要求（要約・DL・クイズ・余白メモ・チャット・蒸留・分野判定）は各Controllerが `activeRequestId` を自前で持ち、ノート切替で無効化する。Vault単位の要求（旧補記一覧・削除・フォルダ一覧・痕跡の整理と退避・**冊子の束と扉**・**分野の索引**）は `NoteSessionCoordinator.vaultGeneration` を共有し、Vault切替でだけ無効化する。混ぜられないのは、旧補記の管理画面がノートと無関係で、ノートを開き直しただけで一覧が消えるのは誤りだからである。冊子も同じ理由でVault単位に置く — ノートへ渡って戻るたびに束が消えるのは、機能の定義そのものに反する。`vaultUri` の比較で代用しないのは、A→B→A と選び直したときに同じ値になり検出できないため。
...
要約、関連ノート、セクションAI、クイズ、余白メモ、検索の主要経路では `CancellationException` を再throwし、キャンセルを一般エラーに変換しない。
...
- AIの空応答は要約文・チャット回答・分野判定の応答読み取りで一定の防御がある。
- **プロンプトに書いた契約は検査へ移すのが原則である。** 分野判定は行の全体がIDのときだけ受理し、蒸留は許可集合の外のIDを棄却する。ひとことが持っていた5検査は機能ごと無くなったが、**指示ではなく検査で守る**という形は残っている。
...
- Vaultへの書き込みは、蒸留による既存ノートの上書き（`**` の挿入のみ・削除なし）、`_ReadingTraces` への読書痕跡JSON保存、旧 `_AI補記` ファイルの削除の3つである。**ノート本文へ書き込むのは蒸留の太字化だけ**で、余白メモも読書痕跡も `.md` には触れない。蒸留の上書きは原バイトSHA-256の二重照合と出力ハッシュ検証を通し、中断時は `noBackupFilesDir` の復旧レコードから起動時に復旧判定する。読書痕跡はユーザーの`.md`に触れないベストエフォート設計で、checksum破損検知はあるが復旧ファイルと原子更新は持たない。**痕跡は書き出して読み戻せる**ので、フォルダごと失っても再生成できない文章、つまりユーザーが書いた余白メモを退避できる（§6.12）。**書き出したファイルは平文**で、Vault内へ保存すると Obsidian の同期でクラウドへ渡ることを保存前に明示する。
...
> **全133クラスを機械的に数え直した。** 件数は `testDebugUnitTest` のレポートから採り、説明は手で書いている。説明の側は古くなりうる。
...
BUILD SUCCESSFUL   133クラス・1,508件・failure 0・skip 0・3秒／Lint 0 errors, 0 warnings
```

2026-09-20 に Android Studio 同梱の JBR で実行した。**この回の hint も0件だった** — 依存更新の催促は Lint が持つ最新版情報のキャッシュ次第で出ないことがある。
`lintAnalyzeDebug` が UP-TO-DATE だと前回のXMLが残るので、**数えるときは `--rerun-tasks` を付けて解析が走ったことを確かめる。**
...
**件数の推移はここに積まない。** 282件から1,500件台までの増え方は各PRの1行が [change_history](../dev/change_history.md) にあり、
...
- **機能を畳むと、テストは検査ごと消える。** ひとことの撤去で63件が消え、余白メモの24件が入った。**件数は保証範囲の代理にならない**
...
- `PromptBuilder` の出力契約（クイズ・蒸留は `QuizPromptBuilderTest` / `DistillPromptBuilderTest` でカバー済み。要約・関連・ピッカー・セクション系の経路は未カバー）
...
- Gemini Nanoの**ダウンロードとタイムアウト**（利用可否と生成は §13.5 が覆うが、**生成は11本のプロンプトのうち5本だけ**）
...
現在の1,508テストは、Android依存の薄い純粋ロジックと、Controller間の調停の回帰防止に有効である。**instrumentation 106件が SAF・画像復号・Compose描画・画面遷移・画素・端末AI生成の一部を実機で覆っている**が、**保証範囲は §13.6 のとおり主張より狭い**。ReadingTraceの高優先度3件はこの境界外で見つかったものであり、修正後も**Android側の実挙動は実端末確認でしか担保できない**。Vigilithも状態分離・モーション・配置範囲は純関数で検証しているが、実フレームの見え方、タップ／ドラッグの競合、Snackbar・IME・ReadingTraceとの視覚的な重なり、TalkBackは実機確認が必要。

---

### 13.5 instrumentation の内訳（106件）
...
| `ai/OnDeviceGenerationTest` | 5 | 本番プロンプトでの実生成。要約・クイズ・関連ノート・セクション要約・**分野判定（応答がIDとして読めること）**。母数は11本 | 端末AI（AICore） |
...
| `ui/DistillRangeAdjustUiTest` | 9 | 範囲調整シートの操作（プリセット切替・最初の範囲へ戻す・つまみのドラッグ・端の微調整）と**告知の出方** | Composable の描画結果・シートの操作 |
| `ui/MarginMemoSheetUiTest` | 4 | 余白メモのシート（入力・一覧・削除の確認・保存中も入力が消えないこと） | Composable の描画結果・シートの操作 |
...
- **端末AIの生成は11本のプロンプトのうち5本だけ。** Nano依存12件の内訳は生成5件・計測と診断5件・要約の基準線2件で、
  読書痕跡要約・蒸留・検索picker・再会カードの選別・セクション候補・セクションchatの**6本は未保証**。
  → 2026-08-08 に主張を代表経路へ狭め、未保証をテスト側へ列挙した。
  分野判定は 2026-09-10 に5本目として足し、ひとこと・映し返しの2本は 2026-09-20 に機能ごと無くなった。
...
6. **壊れやすい文字列処理が純粋関数化されている**
   ...
   クイズパース、余白メモの入力整形、タイトル正規化、Markdown解析をAndroid I/Oから分離し、ユニットテスト可能にしている。

7. **Obsidian固有仕様への配慮がある**

   `.md`、wikilinkの別名・見出し・ブロック参照、frontmatter、機能専用フォルダを扱う。
...
| 中 | **AICore の短期回数制限で、要約タブに SDK の英文が出うる** | ノートを開くたびに自動生成が最大4本走る。計測テストでは12回成功の直後の13回目が拒否された。アプリ本体ではまだ観測していない。要約キャッシュが入れば呼び出し自体が減る。直すか見送るかはオーナー判断 |
...
| 中 | **画面の佇まいを判定する工程が、実機検証にしか無い** | 振る舞いはJVM 1,508件と instrumentation 106件が見ているが、「見分けられるか」「手触りがあるか」はどのテストにも掛からない。2026-09-04 に手触りの10件まで実機ケースで判定できたが、机上のレビューでは依然として出ない。実際、冊子の向きと変形の不足も実機で触って初めて分かった。**テストで埋める種類の穴ではないので、工程側に置いたままになる** |
| 低 | **全106件を一度に通した実行は無い** | 実機検証の単位が機能ごとのケース表へ移ったため（→ `docs/review/device_validation/`）。着手した機能のケースは都度通しており、直近は 09-20 の蒸留の自由範囲と 09-18 の要約の保存 |
...
> **ユーザーが書いた言葉に退避手段が無い**（→ 書き出し／読み戻しを実装、
