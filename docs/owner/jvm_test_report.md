...
**作成:** 2026-09-07 / **更新:** 2026-09-14。ブランチ `feature/Fable51_Improvement_Function_No.Z4`、`da0c414` で計測
...
- JVMテストは **122クラス・1,455件**あり、**2.5秒**で終わる。端末もエミュレータも要らない
- 中身は「機能が正しいか」だけではない。**半分近くが「構造・規約・文書が壊れていないか」を見ている**
- 見ている観点は6種類に分けられる。A 純関数、B 非同期と状態、C 組み合わせ、D 見た目の数値化、E 構造と文書、そして今週増えた F 計測器そのもの
...
|---|---:|---:|---|
| `test/.../`（直下） | 41 | 720 | Controller・保存と読込・パーサ。アプリの動きの本体。共有フェイク `FakeVault.kt` を含む |
| `test/.../domain/` | 31 | 409 | 純関数。採点・抜粋・スコアリング・解析・分野判定の規則 |
| `test/.../ui/` | 20 | 186 | 表示ロジックを数値として固定。色・幾何・派生状態。`theme/` 3本を含む |
| `test/.../architecture/` | 22 | 68 | コードではなく規約と文書を守る検査 |
| `test/.../ai/` | 6 | 33 | プロンプト組み立て。端末AIは呼ばない。共有サンプル `PromptSamples.kt` を含む |
| `test/.../testing/` | 5 | 39 | **要約の採点器そのものの検査。** 共有コーパス `FixedCorpus.kt` を含む |
| `test/.../fakes/` | 1 | — | `FakeAiClient.kt`。本体ではなく道具 |
| **合計** | **126** | **1,455** | 実行 **2.5秒**。クラス実行時間の合算。122クラス＋共有ヘルパ4本 |

> 参考: 実機で走らせる instrumentation テストは別に **16ファイル・99件**ある。
> JVMは「端末に触らずに分かること」だけを引き受け、残りを実機へ渡すという分担になっている。
> 2026-09-08 の前回から JVM は115件、instrumentation は3件増えた。増分の中心は冊子の分野判定と要約の採点器である。
...
ここが他所ではあまり見ない部分で、22ファイル・68件ある。発想は一貫している。
...
| `PromptGenerationCoverageTest` | プロンプトを足したら「実生成で通す」か「未保証と明示する」かを必ず選ばせる。13本中5本が実生成、8本が未保証と宣言 |
...
**27ファイルがソースや文書をファイルとして読んでいる。** `walkTopDown`・`readText` を使う。
...
- **実際の描画・タップ・スクロールの手触り** → `androidTest` 16ファイル99件と、Codexの実機検証
- **端末AIの実生成** → `OnDeviceGenerationTest`。Nano非対応端末では `Assume` で skip。13本のプロンプトのうち5本
...
- テスト実行そのものは2.5秒程度。ビルドから通しても20秒に収まるので、コード変更のたびに通すのが前提の速さになっている
