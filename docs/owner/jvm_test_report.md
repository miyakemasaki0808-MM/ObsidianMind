...
**作成:** 2026-09-07 / **更新:** 2026-09-18。ブランチ `feature/Fable51_Improvement_Function_No.ZA`、`03cc839` で計測
...
- JVMテストは **131クラス・1,525件**あり、**3秒**で終わる。端末もエミュレータも要らない
...
|---|---:|---:|---|
| `test/.../`（直下） | 47 | 748 | Controller・保存と読込・パーサ。アプリの動きの本体。共有フェイク `FakeVault.kt` を含む |
| `test/.../domain/` | 34 | 431 | 純関数。採点・抜粋・スコアリング・解析・分野判定の規則。`markdown/` 1本を含む |
| `test/.../ui/` | 20 | 186 | 表示ロジックを数値として固定。色・幾何・派生状態。`theme/` 3本を含む |
| `test/.../architecture/` | 24 | 85 | コードではなく規約と文書を守る検査 |
| `test/.../ai/` | 7 | 36 | プロンプト組み立てと生成失敗の判定。端末AIは呼ばない。共有サンプル `PromptSamples.kt` を含む |
| `test/.../testing/` | 5 | 39 | **要約の採点器そのものの検査。** 共有コーパス `FixedCorpus.kt` を含む |
| `test/.../fakes/` | 3 | — | `FakeAiClient.kt`・保存のフェイク・門番を置かない印。本体ではなく道具 |
| **合計** | **140** | **1,525** | 実行 **3秒**。クラス実行時間の合算は2.7秒。131クラス＋共有ヘルパ9本 |

> 参考: 実機で走らせる instrumentation テストは別に **16ファイル・99件**ある。
> JVMは「端末に触らずに分かること」だけを引き受け、残りを実機へ渡すという分担になっている。
> 2026-09-14 の前回から JVM は70件増え、instrumentation は変わっていない。
> 増分の中心は**要約の保存**と**自動生成の門番**で、再会カードの32件は痕跡のテストから移ってきたものである。
...
ここが他所ではあまり見ない部分で、24ファイル・85件ある。発想は一貫している。
...
**30ファイルがソースや文書をファイルとして読んでいる。** `walkTopDown`・`readText` を使う。
