...
|---|---:|---:|---:|
| 本番コード（ファイル） | 129 | **138** | +9 |
| 本番コード（行） | 22,183 | **24,130** | +1,947 |
| JVMテスト（ファイル） | 92 | **97** | +5 |
| JVMテスト（行） | 20,044 | **21,865** | +1,821 |
| JVMテスト（件数） | 1,039 | **1,126** | +87 |
...
**08-29 の蒸留の修正はファイルを増やしていない**（既存の `DistillSourceModel.kt` に装飾の保護を足した）。
...
| | 結果 |
|---|---|
| `testDebugUnitTest` | **1,126ケース全件グリーン** |
| `lintDebug` | **Error 0 / Warning 0**（依存更新系は12 hints。ゲートに載せていない） |
| Kotlin コンパイル警告 | **0**（警告はビルドを落とす設定） |
| `assembleDebugAndroidTest` | 成功 |
| instrumentation 実行 | **全57件を一度に通した実行は無い。** 直近は機能単位で実行している — 画像22/22（08-21）・AI入力予算のBUDGET系（08-22）・再会カードの描画6/6（08-22）・痕跡の退避1/1（08-28）。いずれも Pixel 10 Pro Fold・Android 17 |

**実機確認が済んでいないもの:** Vigilith Phase 3 の目視と、**蒸留の装飾保護（`DIST-20`）**。
後者は 08-29 に入れた修正で、JVMテストは通っているが**実機のSAF経路で保存まで確かめていない**
（本文を書き換える唯一の経路なので、通るまで課題台帳から落とさない）。
...
- Markdownパーサー、**ひとことの応答検証**、クイズ応答パーサー、蒸留の文分割・採点・太字挿入、ReadingTraceのJSON・Controller・相対パス走査、Vigilith起動・表示状態・状態別モーション・配置計算、明暗トークンのコントラストなど、壊れやすい純粋ロジックにはユニットテストが整備されている（**1,126ケース**。内訳は §13.1）。
...
| 区分 | ファイル数 | 行数・件数 |
|---|---:|---:|
| 本番 Kotlin | 138ファイル | 24,130行 |
| ユニットテスト Kotlin | 97ファイル | 21,865行、1,126テスト（テストクラスは94。残り3つは共有フェイク・共有ヘルパ） |
...
│   │       ├── markdown/
│   │       │   ├── InlineMarkdown.kt           # インライン装飾のAnnotatedString生成（蒸留が守る記法の正）
...
├── test/java/com/example/newproject/           # 97ファイル・1,126テスト（内訳は §13.1）
...
1. `buildDistillSourceModel`（`DistillSourceModel`）が本文を、UTF-16オフセット保持＋Markdown構造認識（コードフェンス・テーブル・frontmatter・見出しを除外、インラインコードは太字内許容）で文分割する。**保護範囲（`protectedSpans`）に境界を置かない** — コードスパン・リンクに加え、**斜体 `*…*`・太字斜体 `***…***`・打ち消し線 `~~…~~`**（2026-08-29 追加）。判定は表示側 `inlineMarkdown` の分岐順と空白規則を写している。表示用 `NoteSectionModel` は親子重複・見出しなし0件のため流用しない。**同じ段で粒度も決める** — 60字超の文は `splitSentenceIntoClauses` が読点で句へ割り（下限15字へ届くまで前から積み、末尾の余りは直前の句へ吸収。読点は句に含めない）、鉤括弧の中身は `bracketedTermRanges` が語句候補にする。各候補は親文の範囲（`contextRange`）を保つ。
...
文単位の候補でも起きた。JVMテスト11本を足し、6つの変異すべてで落ちることを確かめてある。
**ただし実機のSAF経路では未確認**（`DIST-20`）。
...
**この一覧は、蒸留が壊してはいけない記法の正でもある。** 書き込み側の保護集合を表示側と別に育てた結果、
斜体・打ち消し線をまたいで太字化する欠陥が残っていた（2026-08-29 に解消 → §6.10・[lessons](../dev/lessons/L51.md) L51）。
**ここへ記法を足すときは、蒸留の保護範囲も同時に数え直す。**
...
| `domain/DistillSourceModelTest.kt` | 36 | 文分割、UTF-16オフセット、コード/テーブル/frontmatter除外、**装飾（斜体・太字斜体・打ち消し線）の対を割らないこと／過剰保護もしないこと** |
...
> **今回も全94クラスを機械的に数え直した。** 前回まで「各行は 2026-08-10 時点のまま、
...
現在の1,126テストは、Android依存の薄い純粋ロジックと、Controller間の調停の回帰防止に有効である。**instrumentation 57件が SAF・画像復号・Compose描画・画面遷移・端末AI生成の一部を実機で覆っている**が、**保証範囲は §13.6 のとおり主張より狭い**。ReadingTraceの高優先度3件はこの境界外で見つかったものであり、修正後も**Android側の実挙動は実端末確認でしか担保できない**。Vigilithも状態分離・モーション・配置範囲は純関数で検証しているが、実フレームの見え方、タップ／ドラッグの競合、Snackbar・IME・ReadingTraceとの視覚的な重なり、TalkBackは実機確認が必要。
...
| 優先度 | 項目 | 現状と影響 |
|---|---|---|
| 中 | **蒸留の装飾保護が実機未検証** | 2026-08-29 に斜体・打ち消し線をまたぐ太字化を直したが、確かめたのはJVMまで。**本文を書き換える唯一の経路**なので、`DIST-20` を実機で通すまで完了にしない |
...
> 画像が未対応（→ 実装・実機確認済み）・**ユーザーが書いた返事に退避手段が無い**（→ 書き出し／読み戻しを実装、
