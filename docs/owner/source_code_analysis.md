...
| 本番コード（行） | 22,183 | **24,168** | +1,985 |
| JVMテスト（ファイル） | 92 | **98** | +6 |
| JVMテスト（行） | 20,044 | **22,064** | +2,020 |
| JVMテスト（件数） | 1,039 | **1,142** | +103 |
...
| | 結果 |
|---|---|
| `testDebugUnitTest` | **1,142ケース全件グリーン** |
...
- Markdownパーサー、**ひとことの応答検証**、クイズ応答パーサー、蒸留の文分割・採点・太字挿入、ReadingTraceのJSON・Controller・相対パス走査、Vigilith起動・表示状態・状態別モーション・配置計算、明暗トークンのコントラストなど、壊れやすい純粋ロジックにはユニットテストが整備されている（**1,142ケース**。内訳は §13.1）。
...
| 区分 | ファイル数 | 行数・件数 |
|---|---:|---:|
| 本番 Kotlin | 139ファイル | 24,168行 |
| ユニットテスト Kotlin | 98ファイル | 22,064行、1,142テスト（テストクラスは95。残り3つは共有フェイク・共有ヘルパ） |
...
├── test/java/com/example/newproject/           # 98ファイル・1,142テスト（内訳は §13.1）
...
JVMテストは10本＋16本を足し、**12の変異**すべてで落ちることを確かめてある。
...
**入れ子は再解釈しない** — `**A *B* C**` の内側の `*` はそのまま見える。
...
| `InlineMarkdownTest.kt` | 14 | 強調、リンク、コード、打ち消し、誤検出防止、**描画範囲が共有トークナイザーの答えと一致すること**、エスケープ |
...
| `domain/DistillSourceModelTest.kt` | 41 | 文分割、UTF-16オフセット、コード/テーブル/frontmatter除外、**装飾（斜体・太字斜体・打ち消し線）の対を割らないこと／過剰保護もしないこと** |
| `domain/DistillTransformerTest.kt` | 5 | オフセット降順の `**` 挿入、太字比率上限、短文例外 |
| `domain/markdown/InlineSyntaxTest.kt` | 8 | **インライン記法の唯一の解釈器**（種別・エスケープ・バッククォートrun・リンク消費・対の探索・空白規則） |
...
> **今回も全95クラスを機械的に数え直した。** 前回まで「各行は 2026-08-10 時点のまま、
...
**続く修正確認レビューで16ケースを足して1,142ケース。** 解釈器を表示と共有した回で、
`InlineSyntaxTest`（解釈規則）と、**表示が本当にその答えを使っているかを入力表で突き合わせる検査**を置いた。
変異は12種すべてで落ちる。
...
現在の1,142テストは、Android依存の薄い純粋ロジックと、Controller間の調停の回帰防止に有効である。**instrumentation 57件が SAF・画像復号・Compose描画・画面遷移・端末AI生成の一部を実機で覆っている**が、**保証範囲は §13.6 のとおり主張より狭い**。ReadingTraceの高優先度3件はこの境界外で見つかったものであり、修正後も**Android側の実挙動は実端末確認でしか担保できない**。Vigilithも状態分離・モーション・配置範囲は純関数で検証しているが、実フレームの見え方、タップ／ドラッグの競合、Snackbar・IME・ReadingTraceとの視覚的な重なり、TalkBackは実機確認が必要。
