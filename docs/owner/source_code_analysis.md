...
**測定日:** 2026-08-17（統計は同日に現行ソースから再測定）

---

## 0. 規模の推移

| 指標 | 前回（2026-08-10） | 今回（2026-08-17） | 増減 |
|---|---:|---:|---:|
| 本番コード（ファイル） | 121 | **125** | +4 |
| 本番コード（行） | 19,787 | **20,938** | +1,151 |
| JVMテスト（ファイル） | 73 | **84** | +11 |
| JVMテスト（行） | 15,157 | **18,346** | +3,189 |
| JVMテスト（件数） | 844 | **966** | +122 |
| instrumentation（件数） | 40 | **42** | +2 |
...
**この1週間で増えたものの中身:** 蒸留の候補粒度（リンクだけの候補除外・長文の句分割・
括弧内の語句候補・候補の非重複保証）と、それに伴う表示単位の是正。

> **テストの数え方に1点ぶれがある。** 実機検証手順を見る `DeviceValidationDocsTest`（3件）は
> 手順書ごとリポジトリ外へ出したため、**作業ツリーで数えると969件・85ファイル**になる。
> 上の表は**リポジトリの実態（追跡分）**を載せている。

## 0.1 検証状態（2026-08-17 時点）

| | 結果 |
|---|---|
| `testDebugUnitTest` | **966ケース全件グリーン**（追跡分。作業ツリーでは969） |
| `lintDebug` | **Error 0 / Warning 0**（依存更新系は12 hints。ゲートに載せていない） |
| Kotlin コンパイル警告 | **0**（警告はビルドを落とす設定） |
| `assembleDebugAndroidTest` | 成功 |
| instrumentation 実行 | **37/37 成功・0 skipped**（2026-08-08 / Pixel 10 Pro Fold・Android 17）。**その後5件増えており、42件での実行は未実施** |

**実機確認が済んでいないもの:** Vigilith Phase 3 の目視、A案／B案／C案の一巡。
**未対応の課題は [_wip/current_issues.md](../_wip/current_issues.md) が正本。**

> **instrumentation が42件あることは、保証範囲が42件ぶん広いことを意味しない。**
...
- Markdownパーサー、**ひとことの応答検証**、クイズ応答パーサー、蒸留の文分割・採点・太字挿入、ReadingTraceのJSON・Controller・相対パス走査、Vigilith起動・表示状態・状態別モーション・配置計算、明暗トークンのコントラストなど、壊れやすい純粋ロジックにはユニットテストが整備されている（**966ケース**。内訳は §13.1）。
...
| ユニットテスト Kotlin | 84ファイル | 18,346行、966テスト（追跡分。テストクラスは82。残り2つは共有フェイク） |
| instrumentation テスト Kotlin | 10ファイル | 42テスト（**2026-08-08 に実機で 37/37 成功・0 skipped**。その後ひとことの作り直しで旧補記の作成テストを一覧・削除の検証へ差し替え、3件増えた。内訳は §13.5） |
...
├── test/java/com/example/newproject/           # 84ファイル・966テスト（内訳は §13.1）
...
**未解決:** 書き込み側の受け入れ条件（復旧レコードの4分岐・外部編集の競合中断・最大サイズの計測）は
**一度も実機で通っていない**。候補提示までは実運用で動いているが、**壊れたときに戻せるかが未確認**である。
JVMテストは復旧レコードの読み書きを覆うが、SAFの truncate 窓と実際の中断は再現できない。
表示単位の是正も実機再確認待ち。
...
> **内訳の各行は 2026-08-10 時点のままで、合計だけを 2026-08-17 に更新した。**
> 蒸留の粒度で増えた分（候補選定・原文モデル・Controller・走査検査）は既存クラスへの追加が多く、
> **1行ずつ数え直していない。** 行ごとの件数は目安として読む。
...
**2026-08-16〜17 の蒸留の候補粒度（リンクだけの候補除外・句分割・語句候補・非重複保証・表示単位の是正）で122ケース増え、現在は84ファイル・966ケース全件グリーン**（追跡分）。
...
現在の966テストは、Android依存の薄い純粋ロジックと、Controller間の調停の回帰防止に有効である。**2026-08-08 に instrumentation 34件が加わり、SAF・画像復号・Compose描画・画面遷移・端末AI生成の一部が実機で覆われた**が、**保証範囲は §13.6 のとおり主張より狭い**。ReadingTraceの高優先度3件はこの境界外で見つかったものであり、修正後も**Android側の実挙動は実端末確認でしか担保できない**。Vigilithも状態分離・モーション・配置範囲は純関数で検証しているが、実フレームの見え方、タップ／ドラッグの競合、Snackbar・IME・ReadingTraceとの視覚的な重なり、TalkBackは実機確認が必要。
...
| 中 | **蒸留の短文例外表示が保存範囲と一致しない** | 位置・語句種別・通常件数・確認見出し・保存完了はPixel実機で修正確認済み。ただし句を選んだ短いノート例外が「最重要の1文」「この1文」と説明する（最新レビュー P2-1） |
| 中 | **蒸留の競合・故障復旧は実機未確認** | 通常の語句書き戻しと成功後の復旧レコード削除までは確認済み。外部編集競合と故障注入後の復旧UIは自動テストだけ |
