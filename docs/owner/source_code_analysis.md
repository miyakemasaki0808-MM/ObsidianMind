...
| 本番コード（行） | 22,183 | **24,407** | +2,224 |
| JVMテスト（ファイル） | 92 | **99** | +7 |
| JVMテスト（行） | 20,044 | **22,178** | +2,134 |
| JVMテスト（件数） | 1,039 | **1,146** | +107 |
...
| | 結果 |
|---|---|
| `testDebugUnitTest` | **1,146ケース全件グリーン** |
...
- Markdownパーサー、**ひとことの応答検証**、クイズ応答パーサー、蒸留の文分割・採点・太字挿入、ReadingTraceのJSON・Controller・相対パス走査、Vigilith起動・表示状態・状態別モーション・配置計算、明暗トークンのコントラストなど、壊れやすい純粋ロジックにはユニットテストが整備されている（**1,146ケース**。内訳は §13.1）。
...
| 区分 | ファイル数 | 行数・件数 |
|---|---:|---:|
| 本番 Kotlin | 139ファイル | 24,407行 |
| ユニットテスト Kotlin | 99ファイル | 22,178行、1,146テスト（テストクラスは96。残り3つは共有フェイク・共有ヘルパ） |
...
├── test/java/com/example/newproject/           # 99ファイル・1,146テスト（内訳は §13.1）
...
JVMテストは計30本を足し、**21の変異**すべてで落ちることを確かめてある。
