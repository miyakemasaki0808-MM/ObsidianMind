...
Nowを閉じた後、再会体験を濃くする拡張（X-2）と、長期運用・テスト容易性のための地ならし（X-3・X-4・X-7・X-8）を行う。
...
### X-7. 蒸留の復旧チェックと補記保存の取りこぼしを塞ぐ（current_issues F-2）
- **なぜここ:** どちらも「失敗したときに後始末が無い」型で、A案で全面的に入れた世代管理の**適用漏れ**にあたる。
  ①`DistillController.checkRecovery()` だけが裸の `scope.launch` で、Jobも `activeRequestId` も持たない
  ②`NoteRepository.createAnnotationFile()` は `createDocument()` 後の書込失敗時に作成済みURIを消さないため、
  画面が「生成失敗」と出しても `_AI補記` に空・部分ファイルが残り得る。
  → [current_issues.md](current_issues.md) 2-7・2-8
- **なぜ安いか:** どちらも既存の `ifCurrent {}` を当てるだけで、新しい仕組みを持ち込まない。
  **ただし補記側の失敗注入テストは、`Uri` の壁があるため X-8 が無いと素直に書けない。** 先にやるなら、
  テストの一部を X-8 まで持ち越す前提を決めておく。
...
- **規模感:** 大。中核の型に手を入れるのでA〜C案と同規模の活動になる。X-7 の完全なテストと L-3 の前提を兼ねる。
