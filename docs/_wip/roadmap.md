...
| 段階 | 内容 | 置き場 |
|---|---|---|
| **0** | **走査を正直にする** — `querySafChildren` が列挙の失敗を空と区別する。`EXTRA_LOADING` の要否も調査。`traverseMarkdownPaths` へ不完全性を伝播し、`collectNotes` が完全性を返す | `data` |
| **1** | 痕跡の列挙API（キャッシュを経由しない新規列挙・`vaultKey` 照合） | `data` |
