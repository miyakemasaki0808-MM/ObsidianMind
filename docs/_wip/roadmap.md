...
| ~~**3**~~ | **一覧画面（削除ボタンなし）** — ✅ 2026-08-01 実装完了・**実機確認待ち**。オプション→「読書痕跡を整理」。候補と保留を理由つきで並べる。Controller は8つ目・**Vault単位** | `ui` |
| ~~**4**~~ | **手動削除** — ✅ 2026-08-01 実装完了・**実機確認待ち**。個別＋候補のみの一括。**洗い出した時点のVaultへ削除する**（キーは相対パスのハッシュなので、別Vaultの同名パスと衝突する） | `ui` |
...
- **段階5（限定自動化）は今回のスコープ外。** 着手条件は §14 のまま据え置く。
  **シャドーモードでは保留がほとんど発生しなかった**（痕跡がまだ少ない）ため、
  **遮断器が実データで働くところを一度も観測していない**。「十分な実績」は未達。
