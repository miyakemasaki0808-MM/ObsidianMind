...
- **予算値の調整: 2026-08-01 に1歩目を実施した（関連ノート 600 → 800）**（→ [ai_input_excerpt](../design/ai_input_excerpt.md) §14）。長く最優先としていた「注意書きが予算の38%」は**文字数の話で、トークンでは1.7%（44トークン）**だったため短縮注意書き案は撤回した。掃引で分かったのは**限界費用が全域でほぼ一定＝トークン側に自然な停止点が無い**ことで、**値は生成時間と推薦品質でしか決められない。** どちらも未計測なので800で止めてある。**残るのは実機体感の確認だけ。** なお beta4 は `maxOutputTokens` の既定値が4096になり現状のままでは全経路が上限を超えるため、**この調整を先に終わらせる**（→ [dependency_policy](../design/dependency_policy.md) §5）。 → [current_issues.md](current_issues.md) AI-1
