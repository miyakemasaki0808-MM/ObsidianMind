...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-1](#book-1-冊子モードの修正にactivityとrouteの寿命競合が残る) | 冊子モードの修正にActivityとrouteの寿命競合が残る | 高 | 背面復帰後の冊子時間が直前ノートの痕跡へ混入する |
...
- **BOOK-1 は、N-12 の実装レビューと修正確認で再現順序を追って見つけた。**
  初回9件は修正確認で解消7件・部分解消2件。新規を含め、現HEADで再現する4件だけへ受付を差し替えた。
...
## BOOK-1. 冊子モードの修正にActivityとrouteの寿命競合が残る

- **出どころ:** 2026-08-30 の修正確認（受付簿 `2026-08-30-booklet-fix-review/*`）。
  前回9件は解消7件・部分解消2件。現在残るのはP1 1件・P2 2件・P3 1件。
- **P1:** 冊子routeのeffectは読書時間をpauseするが、冊子表示中の背面復帰でActivity `onStart()` が
  無条件にresumeする。Activity前景とrouteの停止理由を所有者別に持たない限り、冊子滞在が痕跡へ再混入する。
- **P2:** フェンス終端をprefixだけで判定し、記号後に非空白がある行でも閉じる。routeの回帰テストも
  呼出文字列の存在しか見ず、Back取消・先頭復帰・pause/resumeの最終状態を観測しない。
- **P3:** 正本の最終検証が `c8c8ced`＋未コミット差分のままで、現HEADの修正コミットに合わない。
- **受理条件:** 最新レビュー本文 P1-1〜P3-1を、修正前に失敗・修正後に成功する状態／副作用テストへ落とす。
  JVM・Lint・androidTestに加えて `BOOK-01`〜`BOOK-23` の該当ケースを実機で通す。P1とP2-2が残る間は実機へ進めない。
- **2026-08-30、4件とも修正済み（`f2b844a`・取り下げ0件）。** 停止理由を集合で持つ形にして
  背面復帰が冊子の停止を解かないようにし、フェンスは閉じ行の後ろと字下げまで見るようにした。
  走査だけだった配線の担保は、取消を Coordinator へ、先頭表示を `openFromBooklet` へ動かして
  **結果で観測できる位置**へ移した（→ [lessons](../dev/lessons.md) L55）。
  **残るのは次の修正確認レビューと実機検証。**
- **規模感:** 中。
