...
| [SCAN-1](#scan-1-saf列挙が失敗を空に畳むためノートが黙って欠落しうる) | SAF列挙が失敗を「空」に畳むため、ノートが黙って欠落しうる | 中 | Rediscover の母集団と検索結果が理由なく減る |
...
## SCAN-1. SAF列挙が失敗を「空」に畳むため、ノートが黙って欠落しうる

**N-1 段階0 で直る予定だが、SYNC-1 とは独立の欠陥なので別に起票する。**
掃除を作らないと決めても、この問題自体は残る。

- **現状:** [`querySafChildren`](../../app/src/main/java/com/example/newproject/data/SafDocuments.kt#L45) は
  `contentResolver.query(...)?.use { ... }` なので、**`query()` が `null` を返すとブロックごと飛ばして
  空リストを返す**。「読めなかった」と「本当に空」が区別できない。
- **問題:** `collectNotes` のBFSがこの経路を通るため、あるフォルダの列挙が失敗すると
  **その配下のノートが、走査が成功したまま静かに0件になる**。Rediscover の母集団と
  さがすの結果が理由なく減り、**ユーザーには「そのノートが無い」としか見えない**。
  失敗が痕跡を残さないので、報告されても再現条件を作れない。
- **頻度は未確認。** `query()` が null を返すのはプロバイダ側の異常時なので稀とみられるが、
  **静かに壊れることが問題**であって頻度が低いことは緩和にならない。
- **対応:** 失敗を空と区別して返す（nullable・例外・包む型のいずれか）。
  `DocumentsContract.EXTRA_LOADING`（プロバイダが「まだ読み込み中」を申告する経路）の要否も調べる。
  **既存機能は不完全でも見つかった分で動かし続ける** — 完全性を要求するのは掃除だけにする。
- **規模感:** 小〜中（呼び出し側への波及量が戻り値の形で変わる）。
