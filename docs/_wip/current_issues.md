...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-1](#book-1-冊子モード実装に痕跡遷移境界条件の欠陥が残る) | 冊子モード実装に痕跡・遷移・境界条件の欠陥が残る | 高 | 冊子時間が痕跡へ混入し、戻った後にもAIが起動する |
...
- **BOOK-1 は、N-12 の実装レビューで設計どおり閉じていない順序を再現して見つけた。**
  設計レビューの7件をそのまま持ち越さず、解消5件・部分解消2件を判定したうえで、
  現HEADに残る9件だけへ受付を差し替えた。
...
## BOOK-1. 冊子モード実装に痕跡・遷移・境界条件の欠陥が残る

- **出どころ:** 2026-08-30 の実装レビュー（受付簿 `2026-08-30-booklet-implementation-review/*`）。
  旧設計レビュー7件は解消5件・部分解消2件。残った再現順序と実装で新たに見つけた問題を9件へ整理した。
- **高優先の3件:** ①冊子routeで読書時間を止めず、冊子滞在が直前ノートの痕跡へ混入する
  ②「これを読む」でActivityスコープの `noteListState` を0へ戻さず、旧ノートの途中から開く
  ③遅い本文読込中にBackしても `noteLoadJob` が残り、冊子の背後で履歴・痕跡・要約・関連ノートを開始する。
- **境界の4件:** 長いバッククォート／チルダのコードフェンスを正しく除外できない、`openInputStream()` のnullを
  空本文へ畳んでページをReadyにする、Loading中も「これを読む」が有効、1〜9件でも終端を「10枚」と表示する。
- **文書の2件:** 0件時に冊子へ入るかが正本と実装・実機ケースで逆、索引と機能候補が実装済みの冊子を
  「未実装」と案内している。
- **受理条件:** 最新レビュー本文の P1-1〜P3-2 を修正前に失敗・修正後に成功するテストへ落とし、
  JVM・Lint・androidTestに加えて `BOOK-01`〜`BOOK-22` の該当ケースを実機で通す。P1 3件が残る間は実機検証へ進めない。
- **2026-08-30、9件とも修正済み（取り下げ0件）。** 配線3件は素のJVMで組み立てられない
  `NoteViewModel` の外側にあるため、`BookletRouteContractTest` のソース走査で固定した
  （4通りの変異で落ちることを確認済み）。**残るのは修正確認レビューと実機検証。**
- **規模感:** 中。
