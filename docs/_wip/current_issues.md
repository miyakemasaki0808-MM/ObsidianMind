...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [VAL-1](#val-1-実機検証の元vault復帰が読書痕跡を増やす) | 実機検証の元Vault復帰が読書痕跡を増やす | 中 | 検証だけで元Vaultの訪問回数・履歴が変わる |
...
**高は0件・中が3件・低が4件。ほかに超低が1件。**

**残る課題が増えた理由は3つあり、性質が違う。**

- **VAL-1 は、実機検証後の復帰を永続データまで突き合わせて見つけた。**
  X-11の機能不具合ではなく、共通手順の権限境界と通常のVault切替の副作用が両立しない問題である。
...
## VAL-1. 実機検証の元Vault復帰が読書痕跡を増やす

- **現状:** [`MainActivity.openVault`](../../app/src/main/java/com/example/newproject/MainActivity.kt#L62) は
  SAFでVaultを選ぶたび `loadRandomNote()` を呼ぶ。共通の[実機検証手順](../review/device_validation/README.md)は
  元Vaultへ戻しつつ読書痕跡を増やさないよう求めるが、切替自体がランダムノートを1件開くため両立しない。
- **実測:** 2026-08-30のX-11検証で、元Vaultのノート本文と選択は復帰した一方、
  `_ReadingTraces` 3件に訪問が1件ずつ増え、アプリ履歴にも1件増えた。
- **問題:** 元Vaultのサイドカー変更は通常の実機検証権限に含めていないのに、準備・復帰の通常操作だけで
  訪問回数と将来の再会カード入力を変える。画面とVault URIだけを見た復帰確認では検出できない。
- **対応候補:** 開始前に `_ReadingTraces` とVault／履歴設定のバイト基準線を取り、まとめて復元許可を得る。
  許可を得ない場合は専用のテストプロファイルか破棄可能データだけを使う。
  アプリ側にDebug専用の無記録切替経路を設ける案は、手順だけで閉じない場合に検討する。
- **受理条件:** 元Vault→一時Vault→元Vaultの一往復後、ノート・`_ReadingTraces`・Vault／履歴設定が
  開始前のバイト基準線と一致する。復元権限がない場合は元データを書き換える経路へ入らない。
- **規模感:** 小〜中。
