...
- **進捗:** 段階0〜3 まで実装済み。**段階3 は実機確認待ち**（初めてUIから到達する）。
  オプション→「読書痕跡を整理」で、候補と**遮断器が保留した一群を理由つき**で並べる。
  **削除ボタンはまだ無い**（シャドーモード）。**残りは段階4（削除を開ける）。**
- **実機確認ポイント（段階3）:** ①オプションから開ける ②候補ゼロのVaultで
  「ありませんでした」と出る（無表示にしない）③Vault切替後に旧Vaultの候補が残らない
  ④同期途中のVaultで保留の理由が読める文言で出る。
