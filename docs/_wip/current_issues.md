...
ダークモード・B案（実機確認済み）・A案／C案（コード上は完了、実機確認待ち） / `feature/Improvement_Function_No.3`
...
- **① 実機確認（A案・C案／ReadingTrace）** — JVMテストで覆えない境界を同じ端末セッションでまとめて確認する。B案の回帰確認は2026-07-27に完了済み。
