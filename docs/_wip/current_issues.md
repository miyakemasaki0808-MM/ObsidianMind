...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-1](#book-1-冊子モードの実機検証が未完了) | 冊子モードの実機検証が未完了 | 高 | 机上で閉じたライフサイクル／SAF境界が実端末では未確認 |
...
- **BOOK-1 は、N-12 の机上レビューを通過し、実機検証だけが残った。**
  再々修正確認で前回4件をすべて解消し、新規指摘は0件。受付簿からは外し、実機待ちとしてだけ残す。
...
## BOOK-1. 冊子モードの実機検証が未完了

- **現在地:** `fe0f05a` の再々修正確認で前回4件はすべて解消し、新規指摘なし。JVM 1,244件・
  Lint・androidTest APK組み立ては成功しており、机上ゲートは通過した。
- **未確認:** `BOOK-01`〜`BOOK-24` と `BookletScreenTest` 13件。特に遅いSAFでの読込中Back、
  冊子表示中の背面復帰、背面で読込完了するSession、回転／Fold、TalkBackは実端末の順序をまだ観測していない。
- **受理条件:** [冊子の実機ケース](../review/device_validation/booklet_mode.md)を共通手順どおり全件実施し、
  instrumentation 13/13成功、元環境復帰・一時物削除までレビュー本文へ記録する。
- **規模感:** 中。
