...
**抽出日:** 2026-07-22（最終更新: 2026-07-27）
**基準:** ReadingTrace（実機確認待ち・サイドカーは schema v2）・Vigilith Phase 3（最終実機確認済み）・
ダークモード（実機確認済み）・A案／B案／C案（いずれもコード上は完了、実機確認待ち） / `feature/Improvement_Function_No.3`
...
### 3-1. 統合テスト不足
- SAF走査・補記保存/削除・Gemini Nano・Compose Navigation・全画面遷移・画面回転/プロセス再生成・連続操作時の競合を絡めたテストがない。`app/src/androidTest` ソースセット自体が存在しない。ローカルユニットテスト425件は純粋ロジックの回帰防止には有効だが、実端末依存の動作は保証範囲外。
...
- **現状:** 値を変えるとライトの見た目が変わるため未修正。[`AppColorContrastTest`](../../app/src/test/java/com/example/newproject/ui/theme/AppColorContrastTest.kt#L127) が既知未達7件の実測値を「失敗させず記録する」形で固定しており、悪化しても改善しても検出できる。
  **裏を返すと、テスト425件が全緑であることは全配色AA準拠を意味しない。**
...
- **① 実機確認（A案・B案・C案／ReadingTrace）** — JVMテストで覆えない境界を同じ端末セッションでまとめて確認する。
  ...
  - **B案（2026-07-27 完了分）**: Vault切替で7 Controller・さがすタブ・当日履歴が消えること、ノート再読込では補記一覧とフォルダchipsが残ること、テーマ切替が即時反映・再起動後も保持されること、蒸留保存後に本文だけが更新されること、補記/クイズのSnackbarと再会カードが従来どおり出ること。
...
- **③ AI入力の切り出し方を見直す**（2-2）— 品質改善だが Nano の入力予算とのトレードオフがあり、
  関連ノートでフォーカスセクション文脈を採る場合は自動起動との噛み合わせが論点。完了した構造改善とは独立。
