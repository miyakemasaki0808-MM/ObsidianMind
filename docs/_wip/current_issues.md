...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-2](#book-2-冊子が通常のノート表示と見分けられず固有の佇まいが無い) | 冊子が通常のノート表示と見分けられず、固有の佇まいが無い | 中 | 「眺める」と「読む」の形は実装済み。**指摘3件も反映済みで、修正確認と実機検証を待つ** |
...
- **現状:** [`BookletScreen`](../../app/src/main/java/com/example/newproject/ui/screen/BookletScreen.kt) は
  ...
  2026-09-02の修正確認で残ったP2 1件・P3 2件は**すべて反映済み（修正確認待ち）**
  （受付は [findings](../review/findings.md)）。
...
- **実装と指摘の反映は済んだ（2026-09-02）。残るのは修正確認と実機。** 上の3件の修正確認が済んでから
  `BOOK-26`〜`BOOK-30`（[実機ケース](../review/device_validation/booklet_mode.md)）へ進む。
