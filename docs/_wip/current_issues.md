...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-1](#book-1-冊子モードで停止中に始まるsessionの寿命が閉じていない) | 冊子モードで停止中に始まるSessionの寿命が閉じていない | 高 | 背面で読込完了したノートが背景時間を読書として記録する |
...
**高は1件・中が2件・低が4件。ほかに超低が1件。**

**残る課題が増えた理由は3つあり、性質が違う。**

- **BOOK-1 は、N-12 の実装レビューと修正確認で再現順序を追って見つけた。**
  前回4件は再修正確認で解消2件・部分解消2件。現HEADで再現する4件だけへ受付を差し替えた。
...
## BOOK-1. 冊子モードで停止中に始まるSessionの寿命が閉じていない

- **出どころ:** 2026-08-30 の再修正確認（受付簿 `2026-08-30-booklet-refix-review/*`）。
  前回4件は解消2件・部分解消2件。現在残るのはP1 1件・P2 1件・P3 2件。
- **P1:** pause理由を所有者別に分けたが、新しい `ReadingTraceController.Session` は理由が残っていても
  `resumedAtMillis` を時刻で初期化する。遅い読込が背面で完了すると背景時間を読書として積む。
- **P2:** 取消本体のfakeテストは入ったが、本番 `openBookletEntry` のJob登録を通らず、
  痕跡・関連ノートも観測しない。登録1行の削除で製品が再発してもゲートは緑のまま。
- **P3:** `BOOK-23` が2行あり、`BOOK-18` は13件ある描画テストを8件とする。正本の検証表も
  途中の引用で切れ、`実機` 行が表外へ出ている。
- **受理条件:** 最新レビュー本文 P1-1〜P3-2を、修正前に失敗・修正後に成功する状態／副作用／文書検査へ落とす。
  JVM・Lint・androidTestに加えて `BOOK-01`〜`BOOK-24` の該当ケースを実機で通す。P1とP2が残る間は実機へ進めない。
- **2026-08-30、4件とも修正済み（`d714341`・取り下げ0件）。** セッションは停止理由が残る間
  区間を開始しない。冊子の読込は開始と追跡を Coordinator の1手（`openBookletRead`）へ閉じ、
  **本番と同じ入口**で痕跡・履歴・要約・関連ノートが1つも始まらないことを観測する。
  ケースIDの重複と instrumentation 件数のずれは `DeviceValidationDocsTest` が落とす。
  **残るのは次の修正確認レビューと実機検証。**
- **規模感:** 中。
