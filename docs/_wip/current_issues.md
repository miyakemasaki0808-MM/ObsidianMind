...
- **前提だと思っていた2つが前提ではなかった。**
  ...
  - **証拠索引は手動削除には不要**（自動化には要る）。手動削除の前に要るのは
    **走査を正直にすること** — 段階0 で完了済み（実機確認まで終わり、台帳からは削除した）
- **進捗:** 段階0〜4 まで実装済み（**手動削除まで完成**）。段階0・3 は実機確認済み。
  **段階4 が実機確認待ち。**
- **一括削除は持たない（外部レビュー指摘で撤去）。** 遮断器はルート直下が静かに欠けた場合を
  塞げないため、一括削除があると §4 が許容する「稀に1件」を超える一括消失が起こりうる。
  **削除は1件ずつ**にしてある。実績が集まったら戻せる。
- **実機確認は必須としない**（オプション機能でクリティカルでないため・オーナー判断）。
- **段階5（自動化）の門は未達。** シャドーモードでは痕跡が少なく**保留がほとんど
  発生しなかった**ため、**遮断器が実データで働くところを一度も観測していない**。
  必要なのは運用期間であって、段階4 が動いたことではない。
- **代償を承知で受けている:** フォルダごと削除した場合の痕跡は掃除されない
  （「同じフォルダから複数件が欠けた」は列挙失敗と区別できないため保留になる）。
  狙いは**日常的に発生する単発の孤児**であって、一括削除の後始末ではない。
- **手動削除まで実装済み。** 残るのは段階5（限定自動化）だけで、これは実績待ち。
