...
- **前提②の設計は 2026-08-01 に確定した** → [design/saf_boundary_gateway.md](../design/saf_boundary_gateway.md)。
  影響範囲の実測（`android.net.Uri` を import するのは全17ファイル。うち `model` 4・`domain` 1 が撤去対象）と
  7段階の段取りまで詰めてある。**着手すればそのまま進められる状態。**
- **規模感:** 大。前提2つが揃ってから。
