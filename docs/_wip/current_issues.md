...
- **2026-08-29、実装した。** `protectedSpans` へ装飾を入れ、文・句の境界を内側に置かず、
  端が内側に入る候補を採らない。判定は表示側の分岐順と空白規則を写した。
  **JVMテスト11本を足し、6つの変異（保護しない・空白規則を外す・エスケープ／コードを見ない・
  重なりで外す・`**` を斜体と読む）すべてで落ちることを確かめた。**
- **残るのは実機確認だけ。** `DIST-20`（装飾を含む本文の候補と保存）。**通るまでこの項目を消さない。**
- **代償:** 装飾の内側にある鉤括弧の語句候補は消える。安全側への縮小として受け入れた。

## BOLD-2. 太字範囲調整の設計契約が未実装・未検証

[distill_range_adjust](../dev/features/distill_range_adjust.md) は設計済みだが、**実装も検証もされていない。**

**契約の中身はここに写さない。** 2026-08-29 のレビューで出た穴（非重複の維持・未選択候補の扱い・
確定範囲の寿命・Vigilith・衝突解消の切り出し・告知・検証）は**すべて設計書が正本**で、
写すと設計書を直すたびにこちらが古くなる。

- **前提:** BOLD-1 を先に直す。設計が「既存の分割器の出力だから安全」を根拠にしているため。
- **消す条件:** Codexの修正確認レビューを通った時点で、受付簿の `2026-08-29-n14-design/` 9行と本項目を同時に削除する。
  **文書を直しただけでは消さない**（別の目で読むまで確定させない）。
- **規模感:** 中。
