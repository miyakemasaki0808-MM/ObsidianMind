...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOLD-1](#bold-1-太字化が既存の斜体打ち消し線をまたいで記法を壊す) | 太字化が既存の斜体・打ち消し線をまたいで記法を壊す | **高** | 保存のたびにノートの描画が変わりうる |
| [BOLD-2](#bold-2-太字範囲調整の設計契約が未実装未検証) | 太字範囲調整の設計契約が未実装・未検証 | 中 | 実装すると保存時クラッシュ・調整消失が起きうる |
...
## BOLD-1. 太字化が既存の斜体・打ち消し線をまたいで記法を壊す

- **現状:** [`InlineSyntax.protectedForSentenceBreaks`](../../app/src/main/java/com/example/newproject/domain/DistillSourceModel.kt#L17) が
  守るのは**コードスパンとリンクだけ**。既存 `**` は範囲ごと差し引かれるが、
  **斜体 `*…*`・太字斜体 `***…***`・打ち消し線 `~~…~~` は保護も除外もされない。**
  一方で表示側の [`inlineMarkdown`](../../app/src/main/java/com/example/newproject/ui/markdown/InlineMarkdown.kt#L34) は
  この3つを解釈する。**書き込み側の保護集合と、表示側の解釈集合がずれている。**
- **問題（再現済み）:** 装飾の内側で文・句が割れ、その候補を保存すると装飾の対応が変わる。

  | 入力 | 保存後 |
  |---|---|
  | `*この段落はとても長い斜体になっていて、読点をまたいで…である*。` | `***この段落はとても長い斜体になっていて**、読点をまたいで…である*。` |
  | `~~この打ち消し線はとても長くて、読点をまたいで…である~~。` | `**~~この打ち消し線はとても長くて**、読点をまたいで…である~~。` |
  | `*斜体の中に句点がある。この二文目も…末尾にしかない*。` | `***斜体の中に句点がある。**この二文目も…末尾にしかない*。` |

- **文字は消えていないが、記法は保全できていない。** 3例目のとおり**句分割を使わない文単位の候補でも起きる**ので、
  長文だけの問題ではない。**このアプリで唯一ノート本文を書き換える経路**で起きることが重い。
- **対応候補:** `parseInlineSyntax` が `*` / `***` / `~~` の対を検出して保護範囲へ入れ、
  ①その内側で文・句を割らない ②候補の端が対の内側に入るものは候補から外す
  （コードスパンと同じ「またぐなら候補にしない」規則へ揃える）。
  **`_…_` / `__…__` は対象にしない** — `some_var_name` を装飾と誤検出して候補を不当に減らすため。
- **代償:** 斜体の中の鉤括弧から取っていた語句候補が消える（保護範囲と重なるため）。安全側の縮小として受け入れる。
- **受理条件:** 斜体・太字斜体・打ち消し線の内外に読点／句点がある本文で候補が壊れない。
  変換後を再パースし、既存装飾の対象文字列が変わらない。実機で保存まで確認する。
- **規模感:** 小〜中。**実機確認は必須**（本文書き換え経路）。

## BOLD-2. 太字範囲調整の設計契約が未実装・未検証

[distill_range_adjust](../dev/features/distill_range_adjust.md) は設計済みだが、次の契約は
**まだ実装も検証もされていない。** 2026-08-29 のレビューで設計側の穴として出たもので、
実装時に落とすと画面と保存内容がずれる。

- **非重複条件を、選択集合を変えるすべての入口で維持する。**
  [`saveSelection`](../../app/src/main/java/com/example/newproject/controller/DistillController.kt#L308) は
  `applyDistillBold` を **Main で try なしに同期呼び出し**しているので、部分重複は例外ではなく**クラッシュ**になる。
  同一範囲は `distinct()` に畳まれ、選択件数と変更件数が食い違う。
- **未選択候補の範囲変更で、他候補の選択状態を変えない。**
- **確定範囲はシートを閉じても保持する。** 破棄の契機はキャンセル・再解析・ノート切替・保存完了だけ。
- **調整シート表示中は常駐Vigilithを隠す。** 現在 `isBlockingOverlayVisible` は
  [`uiState.isSectionChatSheetVisible`](../../app/src/main/java/com/example/newproject/ui/vigilith/VigilithState.kt#L49) 1本しか見ていない。
- **UI操作に自動検証を置く。** 「開く→変更→閉じる→再度開く」「チェック操作でシートを誤って開かない」「重なり解消の告知」。
- **調整した範囲が保存出力へ届くことを検査する。** 「未調整なら v1 と同一」だけでは、
  保存経路が提案範囲を読み続けていても緑のまま通る。**保存直前の非重複ガードも状態で確かめる。**
- **衝突解消を純関数へ切り出し、範囲変更とチェックの両方から通す。** 入口ごとに書くと片方が取り残される。
- **重なり解消の告知は状態として持つ。** 外れる候補はシートの裏にいるので、一時的な通知では届かない。
  消える契機は「次に選択集合か確定範囲が変わったとき」で、時間では消さない。
- **規模感:** 中。BOLD-1 を先に直すことが前提（設計が「既存の分割器の出力だから安全」を根拠にしているため）。
