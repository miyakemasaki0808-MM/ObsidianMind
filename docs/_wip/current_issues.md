...
| 低 | ダークモードの最終実機確認が未完了 | 実機QA | 暗所での可読性・システムバー・Vigilith haloの品質が未確定 |
| 低 | エラー通知の握りつぶし（フォルダ取得失敗・補記削除失敗等） | 堅牢性 | 無言の失敗 |
| 低 | AI状態の一時エラーと非対応の同一視 | UX | 原因の区別ができない |
| 低 | `fractionBetween` のユーティリティ所在が曖昧 | リファクタ | 将来の二重定義リスク |
| 低 | **ライトの弱い文字6色がAA未達** | a11y | 2.46〜4.43。ダークは全て基準内 |
| 低 | 無彩色グレーが5段階あり意味的な区別がない | リファクタ | ダーク値を5つ決める必要が出る |
| 低 | Vigilith `Summarizing` モードの輝度差テストが未固定 | テスト | 意図しない回帰の検出漏れ |
...
すべて解消し、本ファイルからは削除した（内容は [change_history.md](../change_history.md) PR #35、
設計判断は [reflect_reading_trace](../design/reflect_reading_trace.md) 判断3・判断4、
Vault照合の作り直しは [bugfix_reports](../bugfix_reports.md) #4 に残る）。

ただし SAF実装のVault照合と Activity lifecycle を通した pause/resume は JVMテストの範囲外で、
...
### 3-5. ダークモードの最終実機確認
- **実装・自動検証済み:** オプション画面の明示トグル、SharedPreferencesへの永続化、全画面の
  明暗トークン、Material3面、システムバー、Vigilith haloを実装し、全378件のJVMテストが通過。
  Vigilith Phase 3のタップ／ドラッグ、Fold、Snackbar、IME、TalkBackの最終実機確認は完了済み。
...
- `Float.fractionBetween()` が `VigilithOpeningMotion.kt` に `internal` 拡張関数として定義されており、同パッケージ内の他ファイルから呼べてしまう。`VigilithMascotMotion.kt` は現状使っていないが、将来の二重定義や意図しない依存のリスクがある。
- 対応候補: 共通ユーティリティに移すか、`private` に変更して外部依存を断つ。

### 3-7. Vigilith `Summarizing` モードの輝度差テスト補強
- `VigilithMascotMotionTest` で `Summarizing` の `summaryGuideFraction` 動作は検証済みだが、`lensGlowAlpha` が Idle より強い（ベース 0.18 vs 0.08）ことの数値固定テストがない。意図的な輝度差が将来の変更で崩れても検出できない。
...
- **現状:** 値を変えるとライトの見た目が変わるため未修正。`AppColorContrastTest` が実測値を固定しており、
  悪化しても改善しても検出できる。
...
- ~~**E. Vigilith周辺のリファクタリング**（旧2-6〜2-8）~~ ✅ 完了（2026-07-26・R-1〜R-3）
  → [theme_and_ui_refactor](../design/theme_and_ui_refactor.md)
