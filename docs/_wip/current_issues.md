...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOOK-2](#book-2-冊子が通常のノート表示と見分けられず固有の佇まいが無い) | 冊子が通常のノート表示と見分けられず、固有の佇まいが無い | 中 | 「眺める」と「読む」の形は実装済みだが、**コードレビューP2 2件の修正待ち** |
...
- **現状:** [`BookletScreen`](../../app/src/main/java/com/example/newproject/ui/screen/BookletScreen.kt) は
  ...
  ただし2026-09-02のコードレビューでP2が2件残った（受付は [findings](../review/findings.md)）。
  - **最後のノート紙にも縁が2枚出る。** `BookletPage` が位置に関係なく
    `hasPagesBehind = true` を渡し、10枚目や3本Vaultの3枚目でも存在しない背後紙を描く。
  - **回帰検査が実際の配線を固定していない。** `BearingChannelTest` はコメントを除去するが
    importと未到達の関数を残すため、役割を直書きへ戻したり縁の呼び出しを外したりしても通り得る。
...
- **実装は入ったが、コードレビュー修正待ち（2026-09-02）。** 上のP2 2件を修正確認してから
