...
- **状態:** **再修正済み・修正確認待ち。** 2026-08-29 の再修正確認レビューで出た P1-1・P1-2 を直した。
  前回の P1-1・P2-1・P2-2・P3-1 はそのレビューで解消判定。
- **直した中身:**
  - **入れ子を保つようにした** — 共有解釈器が強調の内側をもう一度解釈し、表示側も再帰的に描く。
    候補が既存装飾を丸ごと含んでも、保存後に斜体・打ち消し線・コード・リンクが表示から消えない。
    あわせて「長い記号で閉じられなければ同じ位置で短い記号を試す」を入れ、
    `***斜体*。**` に余分な `*` が出ないようにした
  - **走査を入力サイズに比例させた** — 位置ごとの一覧探索をカーソルへ、文字ごとの保護範囲の
    List生成を併合済みの並びへ、行ごとの全記法フィルタを行バケットへ変えた。
    28,000リンクの最大サイズ入力で `buildDistillSourceModel` が **7,941ms → 19ms**
- **検証:** JVM 1,143件グリーン。**17の変異**すべてで落ちる。
  上限テストは**リンク密・既存太字密**の入力を別に持つ（→ [lessons](../dev/lessons/L52.md) L52）。
