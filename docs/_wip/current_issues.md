...
- **割り当ては決着した（2026-09-03）。** 動きの行を**「誰が動きを起こすか」で二分**し、
  ...
  **実装済み。残るのは実機での手触り確認だけ。**
- **実装レビュー（2026-09-03、`7506a7e`）の3件は修正済み。**
  - **P2-1:** 束の世代を `BookletState.Open` へ載せ、**最終状態だけ**で再生を決める形にした。
    同期／遅延の引き直し・初回・往復・扉の更新を `BookletRestackTest` が固定する（→ [L58](../dev/lessons/L58.md)）。
  - **P3-1:** 種別引数の走査を、**定数の直後が引数の区切りであること**まで見る形へ戻した。
    レビューが通した変異を含む3種で落ちることを確認済み（→ [L55](../dev/lessons/L55.md)）。
  - **P3-2:** 経路で分けて仕様化した。指に追従する変化は設定の対象外、時間で進む積み直りはOS設定に従う。
    倍率0と1の差と、こちらが倍率を上書きしていないことを検査で固定した。
- **やらないこと:** **演出に「これは冊子だ」と言わせない。** 言い始めると動きが*面の役割*まで運び、
  主役を2つにしないために書体・行間を降ろしたのと同じ理由に触れる。
  手触りは**意味を運ばない**ぶんだけ足せる、という線引きを保つ。
- **残っている作業:** 実機検証 `BOOK-31`〜`BOOK-39` のみ。**手触りの見え方はここでしか判定できない。**
