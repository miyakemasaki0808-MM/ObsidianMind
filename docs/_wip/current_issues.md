...
- **状態:** **実装済み・修正確認待ち。** 2026-08-29 のレビューで出た P1-1・P2-1・P2-2 を直した。
  `domain/markdown/InlineSyntax.kt` に**表示と蒸留が共有する唯一のインライン解釈器**を作り、
  表示側（`inlineMarkdown`）はそれを使って描くだけにした。P3-1（本数の記載）は訂正済み。
- **直した中身:**
  - **解釈規則を1つにした** — バッククォートは開いた数で閉じ、エスケープは対を開かず、
    リンクは構文全体を消費し、対の探索もリンクとコードを飛ばす。表示側の挙動も同じ規則へ揃えた
    （`\*` はリテラルになり、`` ``a*b`` `` が1つのコードになる）
  - **走査の単位を表示側のブロックへ揃えた** — 段落は連結、**リスト項目と引用行は1行ずつ独立**。
    連結していたために別々に描かれる行のあいだで偽の対ができ、候補が0件になっていた
- **検証:** JVM 1,142件グリーン。**12の変異**（保護しない・空白規則・エスケープ・コード・重なり・`**`・
  バッククォートrun・リンク消費・対の探索・ブロック境界・表示側が解釈器を使わない、ほか）すべてで落ちることを確認。
- **消す条件:** 修正確認レビューを通し、そのあと `DIST-20`（装飾を含む本文の候補と保存）を実機で通す。
  **どちらも済むまで消さない。**
- **契約上の代償:** 装飾の内側にある鉤括弧の語句候補は消える。安全側への縮小として受け入れる。
- **残る制約（欠陥ではない）:** 表示側は入れ子のインライン記法を再解釈しないので、
  `*斜体*` を含む文を太字にするとアプリの表示では内側の `*` が見える。Markdownの対応は変わらない。
