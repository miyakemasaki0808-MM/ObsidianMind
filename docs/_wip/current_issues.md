...
- **進捗:** 段階0（走査を正直にする）と段階1（痕跡の列挙API）は実装済み。
  段階1 では `listKeys` / `loadByKey` を足し、**列挙は索引キャッシュを経由せず毎回読み直す**
  （外部同期で増えたファイルを見落とさないため）。段階1 の実機確認は不要 — UIから到達する
  経路がまだ無く、既存動作を変えていない。**残りは段階2（孤児判定）以降。**
