...
> **ここには未対応のものだけを置く。** 解消したらその場で削除する（残すと未対応課題が埋もれる）。
...
| [VERIFY-1](#verify-1-readingtrace-v1-の実機確認が4件残っている) | ReadingTrace v1 の実機確認が4件残っている | 中 | roadmap N-1 を完了扱いにできない |
...
## VERIFY-1. ReadingTrace v1 の実機確認が4件残っている

- **現状:** 到達率・Lifecycle・Vault分離・検索フォールバックの4件。SAF実装のVault照合と
  Activity lifecycle を通した pause/resume は JVMテストの範囲外なので、**実端末確認だけが担保**になる。
- **確認内容:** ①長大な1ブロックを冒頭だけ見ても到達率100%にならない
  ②背面化・復帰をまたいで読書時間と最深位置が正しい
  ③Vault切替で旧痕跡が新Vaultへ入らない ④検索フォールバックの挙動が画面の文言と一致する。
- **完了条件:** これが済むまで [roadmap.md](roadmap.md) N-1 を完了扱いにしない。
- **規模感:** 小（実機で一巡するだけ）。
