...
| [VERIFY-3](#verify-3-documentref-移行の実機確認が未実施) | `DocumentRef` 移行の実機確認が未実施 | **中** | SAF読み書き経路を広く触っている |
...
- **前提が2つある。①は 2026-08-01 に満たした** — Android 16 で 2/2 成功し、**Runnerが起動しComposeを描画できることが実証された**（土台の故障と切り分けられる状態になった）。
  **②も 2026-08-01 にドキュメント参照側が済んだ** — `NoteFile`・`RelatedNote`・`HistoryEntry` は
  `DocumentRef` になり、`model` / `domain` / `ui` から `android.net.Uri` が消えた。
  **残るのは Vault ルートの `Uri` を controller の引数から外す作業だけ**で、
  こちらは検索・補記の起動経路にしか関わらない。
...
## VERIFY-3. `DocumentRef` 移行の実機確認が未実施

- **現状:** 2026-08-01 に `NoteFile`・`RelatedNote`・`HistoryEntry`・`AnnotationState` の
  ドキュメント参照を `DocumentRef` へ移した。**挙動は変えていない**（参照の持ち方だけの変更で、
  等価性も従来の `Uri.equals` と同じ文字列比較）が、**SAFの読み書き経路を広く触っている**。
- **JVMテストで担保できている範囲:** 487件全件グリーン。重複排除・世代照合・履歴のJSON入出力は
  これで覆えている。**むしろ移行によってJVMで書ける範囲が増えた側**である。
- **実機でしか確かめられないのは、実 `ContentResolver` を通す往復。**
  `DocumentRef.value` は `Uri.toString()` なので、`toUri()` で戻したときに
  同一のドキュメントを指すことが前提になっている。
- **確認したいこと（一巡）:**
  1. Rediscover でノートが開く（`NoteFile` 経路）
  2. さがすタブで検索・ランダムから開く（`RelatedNote` 経路）
  3. 「今日読んだノート」に並び、タップで開き直せる（`HistoryEntry` 経路・**保存済みJSONとの互換**）
  4. 補記を生成し、補記管理画面から個別削除・一括削除ができる（`AnnotationState` / 削除経路）
  5. 関連ノートが出る（候補キャッシュのキーが `DocumentRef` に変わっている）
- **3は特に見る。** 移行前に保存された履歴JSONを読めることが互換の証明になるので、
  **アプリを更新する前の履歴が残っている状態で開く**のが望ましい。
- **規模感:** 小（一巡するだけ）。
