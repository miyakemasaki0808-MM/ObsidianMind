...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOLD-2](#bold-2-太字範囲調整が未検証) | 太字範囲調整（段階1）が未検証 | 中 | 本文書き換え経路の変更が実機で確かめられていない |
...
**実装とJVM・UIテストまで済み、実機検証だけが残っている。**

**契約の中身はここに写さない。** 2026-08-29 のレビューで出た穴（非重複の維持・未選択候補の扱い・
確定範囲の寿命・Vigilith・衝突解消の切り出し・告知・検証）は**すべて設計書が正本**で、
写すと設計書を直すたびにこちらが古くなる。

- **残っているもの:** `DIST-21`〜`DIST-24` の実機検証（本文を書き換える唯一の経路を通る）と、
  実装diffの別の目によるレビュー。
- **消す条件:** 実機検証とCodexの修正確認レビューを通った時点で、
  受付簿の `2026-08-29-n14-design/` 9行と本項目を同時に削除する。
