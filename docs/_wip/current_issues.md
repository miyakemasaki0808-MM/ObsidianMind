...
>
> **番号は再利用しない。** 削除した課題の番号は欠番のままにする（他文書からのリンクが別の課題を指してしまうため）。現在の欠番: 2-6〜2-8（Vigilith周辺のリファクタリング／2026-07-26 完了 → [theme_and_ui_refactor](../design/theme_and_ui_refactor.md)）、3-5（ダークモードの実機確認／2026-07-26 完了 → [dark_mode](../design/dark_mode.md) §6）、3-6・3-7・3-10・3-11（軽量課題の一括消化／2026-07-26 完了）。
