...
| [BOOK-3](#book-3-navhost往復テストが表示中の読書ボタンを一意に選べない) | NavHost往復テストが表示中の読書ボタンを一意に選べない | 中 | ページ位置の再発を自動検知できず、冊子のandroidTest合格ゲートが閉じない |
...
**残る課題が増えた理由は5つあり、性質が違う。**

- **BOOK-3 は、BOOK-1修正後のNG再実機検証で追加instrumentationの失敗を再現した。**
  製品のページ復帰は2枚目・3枚目とも直ったが、Pagerが保持する画面外ページにも同名ボタンがあり、
  回帰テストは往復前に一意性エラーで停止した。
...
## BOOK-3. NavHost往復テストが表示中の読書ボタンを一意に選べない

- **現在地:** `967e8bc` の実機で `BookletNavigationTest` を直接実行すると2件中1件が失敗した。
  3枚目まで進んだ後、表示中と画面外にprecomposeされたページの「これを読む」が2件見つかり、
  `onNodeWithText("これを読む").performClick()` が単一ノードを選べず、ノート遷移前に停止する。
- **製品側は解消済み:** 同じAPKの手動操作では、3/10から通常表示を経て3/10、2/10から2/10へ戻った。
  失敗はページ位置の再発ではなく、テストの観測点にある。
- **対応候補:** 現在ページの読書ボタンを一意に示すページID／test tagをセマンティクスへ付け、
  3枚目の表示中ボタンを指定する。ノード配列の先頭を選ぶ位置依存にはしない。
- **受理条件:** 実機で `BookletNavigationTest` が2/2成功し、3枚目のケースが通常ノート表示を経て
  `3/10ページ` の最終検査まで到達する。Pagerが隣接ページを保持していても対象が一意になる。
- **2026-08-31、修正済み。** 読書ボタンに**どのノートを開くかを名乗らせた**（「〈ノート名〉を読む」）。
  テスト用の印ではなく**読み上げの改善**でもある — 全ページが「これを読む」だと、
  めくっても音声では区別が付かない。往復テストはこの名前で表示中の1件を指す。
  **残るのは実機での再実行**（`BookletNavigationTest` 2件・`BookletScreenTest` 14件）。
- **規模感:** 小。
