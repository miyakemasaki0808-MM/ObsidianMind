...
| [AI-1](#ai-1-関連ノートの抜粋予算600が実測より小さいまま) | 関連ノートの抜粋予算600が実測より小さいまま | 中 | 上げる余地が1,684トークンあるのに使っていない |
...
- **残る問題:** 関連ノートの現ノート文脈は600文字（骨格・冒頭・末尾に配分すると各部が数十文字）で、
  **抜粋化の恩恵がこの経路だけ薄い。** 一方で**上げる余地は1,684トークンぶん実測で確認できた。**
- **対応候補:** 上限を800→1,000の順で単独調整し、都度 `PromptTokenBudgetTest` で余裕を確認する。
  計測器は既にあるので、**変更→計測→判断のループが回せる状態**になっている。
  ただし予算を上げると入力が増えて生成時間も伸びるため、**トークン上限だけでなく体感速度も見る。**
- **測るときの注意:** 余裕は `上限 − 入力 − 出力予約` で見る（`getTokenLimit()` は入出力の合計上限）。
  また `maxOutputTokens` は beta2 の既定値256で、**beta4 へ上げると4096になり余裕が3,840減る**
  → [design/dependency_policy.md](../design/dependency_policy.md) §5。**予算調整と beta4 更新は順序を意識する。**
- **規模感:** 小。数字を変えて計測し直すだけ。実機確認は伴う。
