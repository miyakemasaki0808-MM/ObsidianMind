...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [BOLD-2](#bold-2-太字範囲調整にレビュー指摘と実機未検証が残る) | 太字範囲調整にレビュー指摘と実機未検証が残る | 中 | 重なり告知が誤る／消えるうえ、本文書き換え経路が実機未検証 |
...
## BOLD-2. 太字範囲調整にレビュー指摘と実機未検証が残る

[distill_range_adjust](../dev/features/distill_range_adjust.md) の段階1（プリセット）は
実装済みだが、**2026-08-29 のコードレビューでP2 3件・P3 1件が残った。**

**契約の中身はここに写さない。** 2026-08-29 のレビューで出た穴（非重複の維持・未選択候補の扱い・
確定範囲の寿命・Vigilith・衝突解消の切り出し・告知・検証）は**すべて設計書が正本**で、
写すと設計書を直すたびにこちらが古くなる。

- **残っているもの:** 同一プリセット再タップで告知が消える経路、外された候補のシートで主語が誤る経路、
  太字＋下線を検査していないUIテスト、索引・親機能文書の未実装表記を直し、修正確認を受ける。
  その後、本文を書き換える `DIST-21`〜`DIST-24` を実機検証する。
- **消す条件:** 修正確認レビューと実機検証を通った時点で、
  `BOLD-2` へ統合された受付行と本項目を同時に削除する。
