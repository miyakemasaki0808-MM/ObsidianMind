...
| ID | 課題 | 優先度 | 影響 |
|---|---|:--:|---|
| [SYNC-1](#sync-1-読書痕跡を掃除する手段が無い) | 読書痕跡を掃除する手段が無い（着手は全リセットまで。段階案は**超低**・実測待ち） | 低 | 痕跡を消す手段がユーザー側に無い |
| [SYNC-2](#sync-2-_readingtraces-索引が外部同期の追加を認識しない) | `_ReadingTraces` 索引が外部同期の追加を認識しない | 中 | 再会カードの見逃し・重複作成の余地。**SYNC-1 の前提** |
...
- **やる順:** `SYNC-2の索引更新 → 不在証拠の端末内索引 → シャドー表示 → 手動削除 → 自動化判定`。
  **SYNC-2 が先なのは列挙の完全性のため**（同じ列挙APIを全リセットも使うので、
  索引が古いと「すべて削除」が消し残す）。
- **落とすと事故る点:** 証拠索引は **`vaultKey` で区切る**。区切らないと Vault A の走査が
  B の全ノートを不在として記録し、切替を数回往復しただけで B の痕跡が全滅する。
- **閾値は時間ではなく観測回数で持つ。** TTL 60秒は定期実行ではなく再走査の上限なので、
  経過時間は走査回数の代理にならない。票を入れるのは実際に `collectNotes` が走ったときだけ。
- **いま無いもの:** 痕跡を消す手段がユーザー側に一切ない。`ReadingTraceStore` は
  `load`/`save` のキー単位アクセスしか持たないので、**どの段階でも列挙APIの追加が要る**。
- **着手するのは全リセットまで。段階案（シャドー以降）は超低で、着手の契機は実測。**
  段階案が書いてあることは「やる」を意味しない。進む条件は **PERF-1 の列挙コストが
  実機で体感できたとき**、または**孤児が実運用で目に見えて溜まったとき**。
  自動化が回収する価値は「ユーザーが何もしなくて済む」だけで、
  当初の根拠だったファイル数の増加は**削除しなくても索引分割で解ける**と分かっている。
- **規模感:** 全リセットだけなら小。**段階案まで含めると中**（複数フェーズ）。
  ファイル数の増加そのものは解かない（性能は PERF-1 が持つ）。
