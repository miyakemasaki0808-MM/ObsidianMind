...
**最終更新:** 2026-08-02（`feature/Improvement_Function_No.10`・A11Y-1修正）
...
| [A11Y-1](#a11y-1-バッジの塗りが下部ナビ帯の上で判別できない) | バッジの塗りが下部ナビ帯の上で判別できない（**修正済み・実機確認待ち**） | 低 | 記号は読めるが、存在に気づけない |
...
**2026-08-02 に輪郭線で解消した（実機確認待ち）。** ナビ帯の色やバッジの塗り自体は変えず、
`buttonOutlineOnGradient` と同じ形（`LogoNavy` の輪郭線）を再利用した。
→ [design/theme_and_ui_refactor.md](../design/theme_and_ui_refactor.md) 判断8
