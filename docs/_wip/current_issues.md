...
- **状態:** **再修正済み・修正確認待ち。** 2026-08-29 の再々々修正確認レビューで出た
  未閉じコードrunのP1を直した。それ以前の指摘（解釈規則・ブロック単位・リンク消費・入れ子の保持・
  保護範囲カーソル）はいずれも解消判定。
- **直した中身:**
  - **見つからなかったことを覚える前方探索** — 未閉じの `[` / `[[` / バッククォートが並ぶ本文で、
    開始記号ごとに末尾まで探し直していた。250,000文字で **8,440ms → 11ms**。
    表示側も同じ解釈器を通るので、これはMainを止める時間でもあった
  - **バッククォートを連なり単位で索引した** — 位置ごとに `` "`".repeat(n) `` の検索キーを作っており、
    長さの違う未閉じの連なりが並ぶと**96,002文字で `OutOfMemoryError`**（上限の4割未満）。
    同じ長さの連なりだけを閉じにし、無ければ連なりごと越える。**開きより長い連なりの部分一致もやめた**
  - **保護範囲を単調カーソル越しにだけ読む** — 文字ループはカーソル化していたが、その外側で
    候補ごとに一覧を先頭から `any` していた。装飾文32,000件で **5,156ms → 36ms**。
    **用途ごとに別のカーソルを持つ**（端点照合は文末まで進むので、句分割へ渡すと必要なスパンを追い越す）
- **検証:** JVM 1,147件グリーン。**24の変異**すべてで落ちる。上限テストは
  **文字数・記法数・候補数×保護範囲数**の3系統を持ち、時間差が出ない経路は
  `DistillProtectedScanTest` が形で縛る（→ [lessons](../dev/lessons/L52.md) L52）。
- **消す条件:** 修正確認レビューを通し、そのあと `DIST-20`（装飾を含む本文の候補と保存）を実機で通す。
  **どちらも済むまで消さない。**
- **契約上の代償:** 装飾の内側にある鉤括弧の語句候補は消える。安全側への縮小として受け入れる。
