...
- **計測手段は 2026-08-01 に確定した。** `countTokens()` / `getTokenLimit()` は
  **現在使っている beta2 に既に存在する**（AARを展開して確認済み。SDK更新は不要だった）。
  計測器も同日 `PromptTokenBudgetTest` として入っている
  → [design/ai_input_excerpt.md](../design/ai_input_excerpt.md) §13。
  **したがって残っているのは「実機で回して数字を見る」だけ。**
- **判断に使う値を間違えない:** `getTokenLimit()` は**入力と出力の合計上限**で、
  `countTokens()` が数えるのは入力だけ。余裕は `上限 − 入力 − 出力予約` で見る。
  入力だけを見て余裕があると読むと、押し出されるのは生成側になり
  「応答が途中で切れる」という別の症状として出る。
- **対応候補:** 関連ノート専用の短縮注意書きを持たせる（候補側は既にID提示で構造化されているため、
  注意書きを削っても誤読しにくい）／上限を800→1,000の順で単独調整する。どちらも計測結果が先。
  **D-1（プロンプトログ）は前提ではなくなった** — 予算判断に必要な数字は計測器が出す。
