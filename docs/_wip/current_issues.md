...
ダークモード（実機確認済み）・A案／C案（いずれも実機確認待ち） / `feature/Improvement_Issue_No.2`（`76368c3` 時点）
...
> 実施計画は **[§5 改善活動（B案）](#5-改善活動b案)** を参照。下表の「計画」列がどの案で解消するかを示す。
> **A案（非同期の境界）と C案（入出力の境界）は 2026-07-26 に完了**したため、対象だった課題は本ファイルから削除した。
...
| 中 | **2-10 `NoteViewModel` が依存を内部生成しておりテスト不能** | テスト容易性 | Controller間の調停が410件の検証範囲外 | B |
...
**B案でも動かない軸:** アクセシビリティ（3-8・3-9）と開発・リリース運用（3-12〜3-14）。
...
- **問題:** 差し替え口が無いため、**JVMテスト410件が1件も `NoteViewModel` を通っていない**。Controller間の調停ロジック（蒸留保存後の状態維持・Vault切替時の一斉リセット等）は最も壊れやすい部分なのに無検証。
...
### 3-1. 統合テスト不足
- SAF走査・補記保存/削除・Gemini Nano・Compose Navigation・全画面遷移・画面回転/プロセス再生成・連続操作時の競合を絡めたテストがない。`app/src/androidTest` ソースセット自体が存在しない。ローカルユニットテスト410件は純粋ロジックの回帰防止には有効だが、実端末依存の動作は保証範囲外。
...
- **現状:** 値を変えるとライトの見た目が変わるため未修正。[`AppColorContrastTest`](../../app/src/test/java/com/example/newproject/ui/theme/AppColorContrastTest.kt#L127) が既知未達7件の実測値を「失敗させず記録する」形で固定しており、悪化しても改善しても検出できる。
  **裏を返すと、テスト410件が全緑であることは全配色AA準拠を意味しない。**
...
7. **「整理した」と「境界が効いている」も別**（2026-07-26 追加） — パッケージのレイヤー別整理（PR #37）は正しい方向だったが、依存の**向き**は制約されないままで循環が4組残った（2-9）。同様に、コントラストテストが全緑でもAA準拠は意味しない（3-8）し、テスト410件が全緑でも ViewModel は1行も通っていない（2-10）。**「何を保証していないか」を測定と同じ場所に書く**と、この型の誤読を防げる。
...
2026-07-26 の品質総評（7.2 / 10）を受けた実施計画。**残るのはB案（依存と状態の境界）だけ。**

> **A案（非同期の境界）と C案（入出力の境界）は 2026-07-26 に完了した。**
> A案＝Vault世代の採番・全4経路の世代照合・削除失敗件数の表示。
> C案＝用途別の読込予算・累計回数の分離（schema v2）・保存スコープと保存結果の扱い。
> 対象課題はいずれも本ファイルから削除済み。判断の記録は
> [architecture.md](../design/architecture.md)（世代IDの二層化）と
...
- **やること:**
  1. **依存注入:** `ViewModelProvider.Factory` かテスト用コンストラクタで Repository / AiClient / UseCase / 永続化 / `CoroutineScope` を注入可能にする。これで**A案でJVMテストに乗らなかった経路（`loadFolders`・補記一覧/削除）の検証を `NoteViewModel` 越しに書ける**ようになり、Vault切替とController調停の統合テストが初めて成立する（現在410件中0件）
...
「A・C案後」はコード上の変更が入った時点の見込み値で、**実機確認前なので確定ではない**。

| 評価軸 | 当初 | A・C案後（見込み） | B案後（見込み） | 効く案 |
|---|---:|---:|---:|---|
| 可読性 | 7.5 | 7.5 | 8.0 | B |
| 保守性 | 6.5 | 6.5 | 7.5 | B |
| 拡張性 | 6.5 | 6.5 | 7.5 | B |
...
B案では**アクセシビリティ 6.5 と開発・リリース運用 7.0 が動かない**。どちらも独立して進められる。
...
> **品質の底上げは §5 の B案（＋枠外のD・E）に集約した。**
> ここに残すのは、品質軸の点数では測れない「実機確認」と「機能側の判断」だけ。
> 記号が §5 と衝突しないよう ①〜④ で振る。

- **① 実機確認（A案・C案／ReadingTrace）** — JVMテストで覆えない境界を同じ端末セッションでまとめて確認する。
...
- **③ AI入力の切り出し方を見直す**（2-2）— 品質改善だが Nano の入力予算とのトレードオフがあり、
  関連ノートでフォーカスセクション文脈を採る場合は自動起動との噛み合わせが論点。**B案とは独立。**
